...
### 배포 정본과 백업 — 재현 가능한 첨삭 모델

실제 배포되는 자체 첨삭 정본은 3B 계열의 delivery-s 라인(`careertuner-e-correction-3b:latest`)입니다. 이 정본은 가중치 파일에만 의존하지 않고 **재료로부터 재현 가능한 상태**로 관리합니다.

- 학습 산출물인 LoRA adapter 실물(약 57MB)과 학습 데이터셋을 함께 백업해 두었습니다.
- 백업된 base + adapter를 다시 로드하면 배포 정본을 **정확히 복원**할 수 있습니다. 즉 E 첨삭 모델은 서빙 블롭이 사라져도 어댑터·데이터셋으로 되살릴 수 있는 재현 가능한 트랙입니다.

이는 앞의 "없는 사실을 만들지 않는다" 원칙과 별개의 운영 속성입니다 — 사실 날조를 막는 프롬프트·스키마·파서 삼중 방어는 그대로 유지하면서, 모델 자체의 감사·복원 가능성을 재료 백업으로 확보한 것입니다.
