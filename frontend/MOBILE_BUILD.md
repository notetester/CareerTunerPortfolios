...
웹 데모(`https://notetester.github.io/CareerTunerPortfolios/`)와 **같은 mock 빌드를 설치형 앱으로** 한 번에 만든다.
...
- **네이티브 OAuth callback**: `https://careertuner.kro.kr/auth/callback` verified App Link만 사용한다.
  `careertuner://auth/callback`은 다른 앱도 같은 스킴을 등록할 수 있으므로 파서에서 거부한다.
- **네이티브 소셜 계정 연결 결과**: `https://careertuner.kro.kr/profile/detail` verified App Link만 사용한다.
...
adb shell am start -W -n com.careertuner.app/.MainActivity \
  ...
  -d "https://careertuner.kro.kr/auth/callback?error=social_login_cancelled"

adb shell am start -W -n com.careertuner.app/.MainActivity \
  -a android.intent.action.VIEW \
  -d "https://careertuner.kro.kr/profile/detail?socialLinked=KAKAO"
```
...
```bash
curl -fsS https://careertuner.kro.kr/.well-known/assetlinks.json
...
- `App/App.entitlements`: `applinks:careertuner.kro.kr`
...
curl -i https://careertuner.kro.kr/.well-known/apple-app-site-association
