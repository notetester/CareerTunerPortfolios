...
export function ApplicationListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return applicationCases.filter((item) => {
      const matchKeyword =
        !keyword ||
  ...
  }, [applicationCases, search, statusFilter]);
        ...
        {loading ? (
          ...
          <Card className="border-dashed border-slate-300 bg-white">
            <CardContent className="flex flex-col items-center gap-4 p-10 text-center">
              ...
              <div>
                <div className="font-semibold text-slate-900">지원 건이 없습니다</div>
                <p className="mt-1 text-sm text-slate-500">새 지원 건을 만들고 공고문을 등록하세요.</p>
              </div>
              <Button className="bg-blue-600 text-white hover:bg-blue-700" onClick={() => navigate("/applications/new")}>
                <Plus className="size-4" />
                새 지원 건
