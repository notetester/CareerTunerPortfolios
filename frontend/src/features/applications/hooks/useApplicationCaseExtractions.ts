...
export function useApplicationCaseExtractions(applicationCaseIds: number[], enabled = true) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        ids.map(async (id) => [id, await getLatestApplicationCaseExtraction(id)] as const),
      );
      setExtractions(Object.fromEntries(results) as ExtractionMap);
    } catch (err) {
  ...
  const retry = useCallback(async (applicationCaseId: number) => {
