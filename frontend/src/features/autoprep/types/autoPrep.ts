...
/** 6파트 표시 메타(키 → 한글 라벨/아이콘/담당). */
export const PREP_PARTS: Record<string, { label: string; icon: string; part: string }> = {
  PROFILE: { label: "프로필·역량 정리", icon: "👤", part: "A" },
  JOB: { label: "공고 분석", icon: "📄", part: "B" },
  FIT: { label: "적합도 분석", icon: "🎯", part: "C" },
  WRITE: { label: "자소서 교정", icon: "✍️", part: "E" },
  INTERVIEW: { label: "예상 면접 질문", icon: "🎙️", part: "D" },
  COMMUNITY: { label: "커뮤니티 큐레이션", icon: "💬", part: "F" },
};
