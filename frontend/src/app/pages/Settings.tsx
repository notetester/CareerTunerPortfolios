            ...
            <Card className="border border-border bg-card">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="text-xs text-muted-foreground">
                  현재 상태: {consent?.aiDataAgreed ? <Badge className="bg-accent-soft text-accent-2">동의</Badge> : <Badge className="bg-muted text-muted-foreground">미동의/철회</Badge>}
                </div>
              </CardContent>
            </Card>
...
function ConsentToggle({
  ...
  return (
    <div className="rounded-xl border border-border p-4">
      <label className="flex items-center justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-foreground">
            {term.title} <span className="text-xs font-normal text-muted-foreground">{term.version}</span>
          </div>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            {term.required
              ? <Badge className="bg-blue-100 text-blue-700">필수</Badge>
              : <Badge variant="outline">선택</Badge>}
            <span className="text-xs text-muted-foreground">시행일 {term.effectiveDate}</span>
          </div>
        </div>
      ...
      <Link className="ml-3 text-xs font-semibold text-muted-foreground hover:text-primary hover:underline" to={term.documentHref}>전문 열기</Link>
      <p className="mt-3 text-xs leading-5 text-amber-800">{term.restriction}</p>
      {open && (
        <ul className="mt-2 space-y-1 rounded-md bg-muted p-3 text-xs leading-5 text-muted-foreground">
...
function ConsentHistory({ history, filter }: { history: ConsentStatus["history"]; filter: ConsentType[] }) {
  ...
  return (
    <Card className="border border-border bg-card">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base"><History className="size-4 text-muted-foreground" />최근 동의 이력</CardTitle>
      </CardHeader>
      <CardContent>
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">아직 기록된 동의 변경이 없습니다.</p>
        ) : (
            ...
            {rows.map((row) => {
              ...
              return (
                <div key={row.id} className="flex flex-col gap-2 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="font-semibold text-foreground">{term.title}</div>
                    <div className="mt-1 text-xs text-muted-foreground">
                      {row.consentVersion || term.version} · {timestamp ? new Date(timestamp).toLocaleString("ko-KR") : "시각 정보 없음"} · {row.source ?? "USER"}
                    </div>
                  </div>
                  <Badge className={row.agreed && !row.revokedAt ? "bg-accent-soft text-accent-2" : "bg-muted text-muted-foreground"}>
