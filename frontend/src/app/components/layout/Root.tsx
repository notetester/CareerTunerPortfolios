...
export function Root() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className={`flex-1 ${showMobileNav ? "pb-[calc(56px+env(safe-area-inset-bottom))] lg:pb-0" : ""}`}>
        <Outlet />
      </main>
      {!isApplicationDetail && <Footer />}
      {/* 하단 탭이 있는 모바일에서는 플로팅 챗봇이 탭과 겹치므로 데스크톱에서만 띄운다(모바일은 더보기>고객센터). */}
      {!isAdmin && (showMobileNav ? <div className="hidden lg:contents">{<ChatbotBubble />}</div> : <ChatbotBubble />)}
