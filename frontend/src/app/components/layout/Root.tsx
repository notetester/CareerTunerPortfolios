...
export function Root() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <MfaApprovalWatcher />
      <PlannerFloatingOverlay enabled={isAuthenticated && !isAdmin && !isMobileRoute && consentStatus?.aiDataAgreed === true} />
      {!isMobileRoute && (
        <div className="sticky top-0 z-[60]">
          <ServiceStatusBanners />
          {!isAdmin && <Header />}
      ...
      {!isApplicationDetail && !isAdmin && !isMessenger && !isMobileRoute && <Footer />}
      {/* 하단 탭이 있는 모바일에서는 플로팅 챗봇이 탭과 겹치므로 데스크톱에서만 띄운다(모바일은 더보기>고객센터). */}
      {!isAdmin && !isMessenger && !isMobileRoute && (showMobileNav ? <div className="hidden xl:contents">{<ChatbotBubble />}</div> : <ChatbotBubble />)}
