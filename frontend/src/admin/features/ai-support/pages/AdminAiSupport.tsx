...
import {
  Bot, Sparkles, SearchX, Info, RefreshCw, Check, Tag, LoaderCircle,
} from "lucide-react";
...
/** 챗봇 FAQ fast-path 임계값(백엔드 ChatbotProperties.similarityThreshold 기본값과 일치). */
const THRESHOLD = 0.5;
...
export default function AdminAiSupport() {
  ...
  return (
    <AdminShell
      ...
      title="AI 상담 운영"
      icon={Bot}
      desc="챗봇이 답하지 못한 질문을 군집으로 모아 검토하고, FAQ로 전환합니다."
      actions={actions}
    >
      <div className="ais">
        {/* 상태 필터 */}
              ...
              <>
                ...
                <div className="ais-sec">
                    ...
                    <div className="ais-why__b">
                      {cluster.bestFaqQuestion ? (
                        <>가장 가까운 FAQ <b>“{cluster.bestFaqQuestion}”</b>와의 유사도가{" "}
                          ...
                          <b className="num">{THRESHOLD.toFixed(2)}</b>에 미치지 못해 답변을 보류했습니다.</>
                      ) : (
                  ...
                  <div className="ais-bar">
                    ...
                    <span className="ais-bar__thr" style={{ left: `${THRESHOLD * 100}%` }} />
                  </div>
                  <div className="ais-barx"><span>0.00</span><span>임계값 {THRESHOLD.toFixed(2)}</span><span>1.00</span></div>
                </div>
