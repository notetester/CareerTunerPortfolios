...
test("딥링크는 manifest의 canonical host와 안전한 앱 경로만 허용한다", () => {
  ...
  assert.equal(toAppPath(`https://careertuner.example.com/auth/callback?handoffCode=${code}`), `/auth/callback?handoffCode=${code}`);
  assert.equal(toAppPath("https://careertuner.example.com/auth/callback?error=social_login_cancelled"), "/auth/callback?error=social_login_cancelled");
  ...
  assert.equal(toAppPath("https://careertuner.example.com/community?tab=hot#top"), null);
  assert.equal(toAppPath("https://careertuner.example.com/auth/reset-password?token=email-token"), null);
  assert.equal(toAppPath("http://careertuner.example.com/applications"), null);
  assert.equal(toAppPath("https://careertuner.example.com/applications"), null);
  assert.equal(toAppPath("https://www.careertuner.example.com/applications"), null);
  ...
  assert.equal(toAppPath("https://careertuner.example.com/auth/callback?accessToken=secret"), null);
  assert.equal(toAppPath(`https://careertuner.example.com/auth/callback?handoffCode=${code}&handoffCode=${code}`), null);
  assert.equal(toAppPath("https://careertuner.example.com/auth/callback?handoffCode=short"), null);
  assert.equal(toAppPath(`https://careertuner.example.com/auth/callback/extra?handoffCode=${code}`), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinked=KAKAO"), "/profile/detail?socialLinked=KAKAO");
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinked=NAVER&socialMock=1"), "/profile/detail?socialLinked=NAVER&socialMock=1");
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinkError=social_login_cancelled"), "/profile/detail?socialLinkError=social_login_cancelled");
  assert.equal(toAppPath("https://careertuner.example.com/profile/social-callback?socialLinked=KAKAO"), null);
  ...
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialMock=1"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinked=kakao"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinked=KAKAO&socialLinkError=social_login_failed"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinkError=arbitrary"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinked=KAKAO&next=/admin"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail?socialLinked=KAKAO&socialLinked=NAVER"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profile/detail/extra?socialLinked=KAKAO"), null);
  assert.equal(toAppPath("https://careertuner.example.com/profileevil?socialLinked=KAKAO"), null);
