...
test("Android main은 cleartext·백업을 닫고 시스템 bar 대비를 보장하며 debug overlay만 개발용으로 연다", async () => {
  const [mainManifest, mainNetworkConfig, backupRules, dataExtractionRules, debugManifest, debugNetworkConfig, gradle, styles, stylesV27, stylesV29, mainActivity, nativeShell, nativeConfiguration, pushAdapter] = await Promise.all([
    ...
    readFile(new URL("../android/app/src/main/java/com/careertuner/app/configuration/NativeConfigurationPlugin.java", import.meta.url), "utf8"),
    readFile(new URL("../src/platform/push.ts", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(mainActivity, /registerPlugin\(NativeConfigurationPlugin\.class\)/);
  ...
  assert.match(nativeConfiguration, /Class\.forName\("com\.google\.firebase\.FirebaseApp"\)/);
  assert.match(nativeConfiguration, /getMethod\("getInstance"\)/);
  assert.match(nativeConfiguration, /InvocationTargetException/);
  const firebaseGuard = pushAdapter.indexOf("await isNativePushRegistrationConfigured()");
  const unsafeRegister = pushAdapter.indexOf("await plugin.register()");
  assert.ok(firebaseGuard >= 0 && unsafeRegister > firebaseGuard, "Firebase guard가 PushNotifications.register보다 먼저 실행되어야 한다");
});
