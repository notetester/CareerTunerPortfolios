...
## 파일
- `interview_golden_cases.jsonl` — 1줄 = 1케이스(현재 **20건**). `*.jsonl` 전역 무시에서
...
```json
{
  ...
  "expectedScore": 92,                    // 사람이 매긴 0~100 골든 점수
  "expectedBand": "HIGH",                 // HIGH>=80 | MID 40~79 | LOW<40
...
## 커버리지 (20건)
| 밴드 \ 유형 | TECH | PERSONALITY | SITUATION |
| --- | --- | --- | --- |
| HIGH(>=80) | 001, 002, 007 | 010, 014 | 015, 018 |
| MID(40~79) | 003, 004, 008 | 011, 013 | 016, 019 |
| LOW(<40)   | 005, 006, 009, 020 | 012 | 017 |

- 모든 case 는 한국어(제품이 한국어). 회사·직무를 실제 채용 맥락으로 다양화.
- LOW 극단(020 "잘 모르겠습니다")으로 최하점 clamp/파싱을 검증.
...
## PASS 기준 (D 확정 대기 — 제안값)
> **아래 임계값은 제안(proposal)이며, D 오너가 라이브 A/B 실측으로 비준(ratify)하기 전까지 확정이 아니다.**
...
이유: 면접 채점은 밴드(HIGH/MID/LOW) 경계에서의 안정성이 UX에 직결되므로, 절대 점수보다
...
## 확장 TODO
- 라이브 A/B(양 Ollama 호스트 UP) 실측 후 D 오너가 임계값(1~5)을 비준.
- 40~60건으로 확대 + 밴드 경계(58/62/79/80) 케이스 보강.
- critique/report 프롬프트 계약도 별도 골든셋으로 확장 검토.
