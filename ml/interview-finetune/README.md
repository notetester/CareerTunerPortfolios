...
> **⚠️ 재현성 경고 (현재성 기준: 2026-07-15 문서 갱신, 최종 백업 스냅샷 `origin/dev` `23bb4d22`·2026-07-14 검증):**
> D의 원 학습 JSONL은 `.gitignore` 대상이라 이 저장소에 없고, 4090 클론과 최종 백업 `datasets/` 어디에도
> interview 학습 데이터셋이 **0개**다. 최종 백업에는 `interview-3b` modelfile 2종만 남아 있다. 원본 데이터가
> 남지 않아 **새 clone으로는 2026-06-20 체크포인트를 byte-identical은 물론 기능적으로도 재현할 수 없다.**
> A 프로필 모듈은 추적된 500행 기준선으로 최소 재학습이 가능하지만, D에는 그런 clone 재현 기준선이 없다.
> 잔존 자산은 GGUF 블롭·Modelfile과 이 저장소의 스크립트·60건 골든셋뿐이며, 재학습하려면 관리자 export
> 또는 합성 데이터를 새로 만들어야 한다.
> 최종 백업: [`docs/ai-artifacts/results/2026-07-14-final-model-backup/`](../../docs/ai-artifacts/results/2026-07-14-final-model-backup/)
> (`modelfiles/interview-3b_latest.modelfile`, `interview-3b_q4.modelfile`만 보존).
...
> **원 학습데이터 없음(관리자 export/합성 재생성 필요):** 2026-06-20 서비스 체크포인트에 쓴 messages JSONL은
> 백업되지 않았다. 아래 명령은 파이프라인 골격일 뿐이며, 실제로는 관리자 export(`/api/admin/interview/training/export`)
> 또는 합성 워크플로로 데이터셋을 **새로 재생성**해야 동작한다. 재생성한 데이터는 원본과 동일하지 않다.
