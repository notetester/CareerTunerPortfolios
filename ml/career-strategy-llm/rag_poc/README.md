...
[reports/50](../../../docs/ai-reports/areas/c-career-strategy/reports/50_rag_design_plan.md) RAG 설계의 **오프라인 검색 PoC**. backend 통합 전, 지원 건 1개 기준으로 핵심 가설을 코드로 검증한다.
...
## 검증 목표 ([reports/50](../../../docs/ai-reports/areas/c-career-strategy/reports/50_rag_design_plan.md) → 코드)
...
- **R1b(완료)**: local embedding + cosine vector search. 환경에 numpy/ST/sklearn 부재 → **순수 파이썬 HashEmbedder(결정론·오프라인) 기본**, `CT_RAG_ST_MODEL` 지정 시 sentence-transformers 사용(자동 대형 다운로드 안 함). 동일 scope filter·fixture 유지, **scope filter 를 vector search 전에** 적용. 실제 의미모델(`bge-m3`/`multilingual-e5`)은 환경 구비 시 동일 인터페이스로 교체([reports/52](../../../docs/ai-reports/areas/c-career-strategy/reports/52_rag_local_embedding_poc_result.md)).
- **R2**: `retrievedContext` 를 3B LoRA prompt 에 실제 주입해 A(LoRA) vs B(LoRA+RAG) 오프라인 비교([reports/50](../../../docs/ai-reports/areas/c-career-strategy/reports/50_rag_design_plan.md) §10).
