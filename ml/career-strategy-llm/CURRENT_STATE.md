...
Last updated: 2026-07-14 (`origin/dev` `23bb4d22`)
...
| True External Retrieval RAG | 카탈로그·NCS 같은 외부 지식 corpus를 검색해 C 생성 입력에 주입 | NCS 오프라인 PoC와 3B 생성 A/B 완료. raw 생성 주입은 악화, 결정론 활용은 유지, production 자동 주입은 미연결 |
| 기업 Web Intelligence(B→C) | B가 선택적으로 네이버 검색 근거를 수집해 기업 분석을 만들고, C는 저장된 회사 요약·최근 이슈·면접 포인트를 설명 전용 `companyContext`로 사용 | 구현됨. 기능 기본 ON이나 네이버 키가 없거나 검색 실패 시 B가 공고-only로 degrade; C 판단값에는 미사용 |
| 자격증 시험일정 근거(C) | 국가기술자격 통합 공공데이터 API, 국가전문자격 연간 사전공고 번들, 민간자격 등록 근거를 추천 카드에 연결 | 부분 구현. 키 미설정 시 라이브 근거 수집을 건너뛰고, 종목코드 미매핑·업스트림 장애는 확인 불가, 번들 밖 전문자격·민간 일정은 수동 확인으로 degrade; 적합도 분석은 계속 |
...
- **폴백 체인 = self/OSS→Claude→OpenAI→Mock, 체인 레벨 시간예산으로 유계(2026-07-07, PR #283).** 종전 총 시간예산은 self/OSS(GPU) tier 만 묶어 사용자 대기가 무제한(최악 ~720s)이었다 → C 소유 3개 디스패처(fitanalysis/dashboard/analysis-trend)에 `careertuner.analysis.ai.chain-total-time-budget`(기본 120s, 0=끔) 추가: 앞 tier 예산 소진 시 외부 tier 건너뛰고 즉시 Mock 안전망. 전 도메인 감사([AIDocs 86](../../docs/ai-reports/areas/c-career-strategy/reports/86_ai_fallback_chain_audit_and_c_ai_status.md))에서 타 도메인 CRITICAL(E-correction Mock 부재·throw, admin summarizeMember screen-break, B 예산 0)은 소유자 태스크로 분리.
- **C AI 개선 = 검증-안정·미종결(운영 데이터 게이트).** 모델(3B LoRA 유지)·안전 계층(E1+R3)은 production-ready·재벤치마크 완료. 남은 개선(FP triage 재평가·학습셋 환류·재학습)은 대부분 **운영 gate 데이터 축적 선행 조건**이며 관측 인프라(gate 통계 API #229)는 구축됨. PARSE_FAIL 은 truncation 수리(#229)로 완화. 상세 판정: [AIDocs 86](../../docs/ai-reports/areas/c-career-strategy/reports/86_ai_fallback_chain_audit_and_c_ai_status.md) §4.
- **GPU 공유 방어는 이중 트랙이다(2026-07-02, 옵션2는 2026-07-06 확정).** 서버 쪽=옵션2(Ollama `NUM_PARALLEL=2`/`MAX_LOADED_MODELS=8` — **NP×MLM 그리드로 확정**: MLM=8 이 5모델 상주로 MLM=3 대비 ~7배 처리량, 단 NP=8 은 KV×슬롯이 VRAM 포화시켜 붕괴(무제한 최고 가설 기각)→NP 역U자에서 NP=2 가 안전 정점), 백엔드 쪽=옵션4 GPU permit gate(전 도메인 12지점, **기본 OFF**, PR #222) + 총 시간예산 전 도메인(0=무제한 토글, PR #226). 전부 독립적으로 켜고 끌 수 있으며 기본 상태는 기존 동작과 동일하다. 운용 문서는 CareerTunerAI `docs/ops/4090_{OLLAMA_CONCURRENCY_TUNING,GPU_BACKEND_PERMIT_GATE}.md`, 부하·MLM 결과는 `benchmarks/gpu-concurrency-loadtest/`.
...
- 2026-07-14 같은 데이터·LoRA recipe·golden60으로 7B LoRA까지 학습·비교했다. contract 57→55,
  E1 11→10으로 단일 seed에서 능력 우열은 구분되지 않았고, 7B는 지연 약 1.8배·평가 VRAM 약 2.1배·
  학습시간 약 1.68배가 필요했다.
- `KEEP_3B`는 7B 열등 판정이 아니라 현재 배포 제약에 따른 비용 결정이다. 멀티시드·golden 확대·모델별
  튜닝 없이 일반적인 3B 우월성을 주장하지 않는다.
- Campaign 5의 동일 schema 통제에서는 raw contract가 LoRA 56/60, base 53/60이었고, 게이트 후
  accepted는 LoRA 46/60, base 49/60, E1은 LoRA 11/60, base 5/60이었다. 따라서 schema는 구조,
  E1+R3는 사실 안전, LoRA는 전략 품질이라는 역할로 분리한다. 서로 다른 두 블라인드 LLM 판정자는
  전략 품질 5축에서 LoRA 방향 우위를 일관되게 관찰했지만, 출력 길이·공통 LLM 편향과 단일 평가 세트
  한계 때문에 일반적 우월성으로 확대하지 않는다.
...
- **NCS external retrieval은 오프라인 결정론 PoC와 3B 생성 A/B까지 실행했다.** NCS chunk를 `jobRequirements`로만 분류하고
  userEvidence에서 격리하며, 요구 능력단위·로드맵을 결정론적으로 만들고 R3 gate로 보유단정 유출을 잡는
  테스트 12건이 통과했다([AIDocs report 87](../../docs/ai-reports/areas/c-career-strategy/reports/87_ncs_evidence_grounded_rag_poc.md)).
  이후 synthetic 8케이스×repeat2에서 무주입 16회와 NCS 주입 16회를 비교했다. NCS 주입은 E1 5→6,
  `requirement_as_owned` 0→12로 raw grounding을 악화시켰고, unsafe 주입 출력은 gate가 accepted serving에서
  전부 제외했다. 따라서 “RAG 운영 중”이나 품질 향상으로 표현하지 않고, production 자동 주입은 유지하지 않는다.
- 아래 R2 계열 실측·판단은 retrievedContext/evidence-bucket **prompt augmentation**(이미 입력된 profile/job 의
  재구조화 + 정적 합성 사실)에 대한 것이다.
...
- retrieved-context/evidence-bucket 자동 주입은 production prompt에 연결하지 않는다(`KEEP_RAG_DISABLED` 유지).
  NCS 생성 주입도 기본 비활성으로 두고, 검색 결과는 생성 문장보다 결정론 `jobRequirements`·로드맵 근거로 활용한다.
...
- 2026-07-02 gold label 확정(사용자 승인 하 top-LLM 대체): disagreement13 독립 판정 TRUE 0/13(top-LLM consensus gold 확정, offline over-count 근원=모델 자기신고 metric 합산), A-only TRUE 3건 확정. A-only 후보·판정 근거는 [AIDocs report 80](../../docs/ai-reports/areas/c-career-strategy/reports/80_a_only_baseline_repeat2_judge.md)에 보존했고, TRUE 3건은 **E1/R3 production 코드 replay 테스트로 실검출 증명**했다.
...
- 2026-07-10 기업맥락(`## 기업 맥락` 서빙 프롬프트 섹션) conflation 실측: 함정(프로필에 없는 기업 기술·수치) 포함 8픽스처×유/무 16생성을 이중 판정단(Claude + Codex gpt-5.6-sol)이 독립 판정해 **완전 일치 — 기업맥락 함정 혼동 0/8**. 유일 위반 1건(f7/ctx, missing Redis 보유 서술)은 기업맥락과 무관한 기본 grounding 위반으로 E1 검출 유형. **결론: 기업맥락 연동 유지, 추가 가드 불요**(retrievedContext 주입과 달리 기업맥락은 conflation 을 유발하지 않음 — 역할 라벨이 명시된 소량 구조화 맥락이라는 차이). raw/판정: `docs/ai-artifacts/benchmarks/c-company-context-conflation/`.
- 2026-07-14 Campaign 4·5에서 NCS 생성 주입 악화와 동일 schema base/LoRA 비교를 각각 실측했다.
  최종 결론은 `KEEP_3B`와 production NCS 자동 주입 비활성을 유지하되, LoRA·schema·결정론 gate의 역할을
  분리하는 것이다. 원자료는 `docs/ai-artifacts/results/2026-07-14-campaign{4,5}/`, 누적 해석은
  [AI 종합 기술 보고서 §13.5.10~11](../../docs/ai-reports/areas/shared-ai/portfolio/careertuner-self-ai-model-deep-dive.md)에 보존한다.
