...
- **GPU 공유 방어는 이중 트랙이다(2026-07-02, 옵션2는 2026-07-06 확정).** 서버 쪽=옵션2(Ollama `NUM_PARALLEL=2`/`MAX_LOADED_MODELS=8` — **부하 테스트로 확정**: NP는 무제약 baseline=auto NP1 이 최악이라 np2 채택, MLM은 멀티모델 실측으로 3→8 상향해 다도메인 축출 방지·VRAM(~21GB, ~5모델)이 실질 상한), 백엔드 쪽=옵션4 GPU permit gate(전 도메인 12지점, **기본 OFF**, PR #222) + 총 시간예산 전 도메인(0=무제한 토글, PR #226). 전부 독립적으로 켜고 끌 수 있으며 기본 상태는 기존 동작과 동일하다. 운용 문서는 CareerTunerAI `docs/ops/4090_{OLLAMA_CONCURRENCY_TUNING,GPU_BACKEND_PERMIT_GATE}.md`, 부하·MLM 결과는 `benchmarks/gpu-concurrency-loadtest/`.
