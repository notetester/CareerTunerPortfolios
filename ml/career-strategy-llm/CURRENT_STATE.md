...
- **GPU 공유 방어는 이중 트랙이다(2026-07-02).** 서버 쪽=옵션2(Ollama `NUM_PARALLEL=2` 적용, **값 비확정**), 백엔드 쪽=옵션4 GPU permit gate(전 도메인 12지점, **기본 OFF**, PR #222) + 총 시간예산 전 도메인(0=무제한 토글, PR #226). 전부 독립적으로 켜고 끌 수 있으며 기본 상태는 기존 동작과 동일하다. 운용 문서는 CareerTunerAI `docs/ops/4090_{OLLAMA_CONCURRENCY_TUNING,GPU_BACKEND_PERMIT_GATE}.md`.
