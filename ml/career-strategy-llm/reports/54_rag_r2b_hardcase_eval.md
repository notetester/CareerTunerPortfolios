# R2b — RAG 가 도움 되는 어려운 케이스(hard-case)에서 A/B 재평가 (2026-06-26)
...
## 13. 다음 단계 판단
**판정: 불명확(실측 대기) — 단, R2b 케이스셋은 R2 의 headroom 부재 문제를 구조적으로 해소.** R2 는 A 가 이미 perfect 라 효과를 증명도 반증도 못 했는데, R2b 는 base 가 틀리기 쉬운 8유형(MSSQL/제품코드/유사기술/데이터 도구/회사 사실/자격 catalog)을 넣어 **개선 측정 가능한 케이스셋**을 만들었다. 4090 SSH 복구 시 §7 command 한 줄로 실측 → §8~10 채움.
