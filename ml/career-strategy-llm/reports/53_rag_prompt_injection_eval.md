...
> **실행 상태: 하니스·케이스·테스트 완성 + mock 으로 오프라인 검증 완료. 실제 3B LoRA 호출(A/B 수치)은 4090 SSH 복구 후 실행 — 현재 보류**(아래 §7).
...
## 7. 실행 여부 (실제 A/B 수치는 보류)
- **하니스 오프라인 검증(완료):** `compare_lora_with_rag.py --mock --repeat 2` 정상 — 8 케이스 × A/B × 2 = 32 run, build→mock 출력→`evaluate` 채점→집계 무크래시. RAG 전용 지표가 B(context_overlap>0)와 A(0)를 구분.
- **실제 3B LoRA A/B(보류):** **4090 SSH 가 publickey 거부**(2026-06-26, 풀세션·트리거 키 모두 `Permission denied (publickey)`). SSH 서버는 응답하나 인증 실패 — 재부팅/`administrators_authorized_keys` 리셋 추정. 원격에서 키 재등록 불가 → 실행 보류.
- **복구 후 실행 명령(CareerTunerAI `jobs/open/2026-06-26-rag-r2-prompt-001.json`):**
  ```bash
...
## 8. 지표 결과 또는 실행 대기 사유
- mock(하니스 검증): A·B 모두 success 1.0/json 1.0/CJK 0/hallucinated 0(mock 은 allowed 만 사용), B 의 context_overlap 0.08 vs A 0.0. → **하니스·지표 파이프라인 정상**(실제 품질 비교 아님).
- 실제 A/B 수치: **4090 SSH 복구 후 산출 예정**(위 §7). 본 PR 에는 raw 결과 없음.

## 9. RAG 가 개선한 점 / ## 10. RAG 가 악화시킨 점
- **현재 단정 불가** — 실제 3B LoRA 호출 미실행. mock 은 동일 출력이라 A/B 품질차를 보이지 않음(설계상). 실제 비교는 4090 실행으로.
...
## 13. 다음 단계 제안
- **4090 SSH 복구**(authorized_keys 재등록 — 박스 접근 필요) 후 위 command 로 실제 A/B 실행 → reports/53 갱신(또는 53b 결과 PR).
- 결과에 따라: **유망 → R2b golden 확대 후 R3 backend(Spring AI/LangChain4j) 통합** / 불명확 → retrievedContext 포맷·검색 품질 개선 후 재평가 / 악화 → runtime 통합 보류.
- (환경 구비 시) R1b 의 실제 의미 embedding(bge-m3/e5)으로 retrievedContext 품질을 높여 A/B 재측정.
