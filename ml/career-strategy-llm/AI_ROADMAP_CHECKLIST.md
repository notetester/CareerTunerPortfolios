...
- [x] 완료 — GPU 옵션2: 4090 Ollama 에 `MAX_LOADED_MODELS=3`·`NUM_PARALLEL=2` 적용(2026-07-02) 후 **2026-07-06 부하 테스트로 확정**(baseline/np2/np4 3조합, 오류율 0). 무제약 baseline=auto NP1 이 최악 → 무제약 가설 기각, np2 채택·rollback 없음. 결과 CareerTunerAI `benchmarks/gpu-concurrency-loadtest/`, 갱신 문서 `docs/ops/4090_OLLAMA_CONCURRENCY_TUNING.md`.
...
- [x] 완료 — GPU 옵션2 §6 서버축 부하 테스트(baseline/np2/np4 × 동시성 2·4, A-only 120요청/조합): np2/mlm3 확정. 결과 `benchmarks/gpu-concurrency-loadtest/README.md`.
