...
* 2026-07-02: **post-R3 정식 재벤치마크**(hardcase12×2변형 + A-only60, 각 repeat2, 4090) — 결정론 unsafe 0 유지, B 변형 우위 재확인, A-only 후보 r1∩r2=∅(확률 변동). 동일 시기 GPU 운용 변경: 옵션2(OLLAMA NUM_PARALLEL=2 적용, 값 비확정), 옵션4(백엔드 GPU permit gate, 기본 OFF, #222), 총 시간예산 전 도메인 확장(0=무제한 토글, #226). 관리자 SQL DB-fixture 통합테스트(#221)로 known limitation 1건 해소.
