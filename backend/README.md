...
| 기능 | 켜는 방법 |
| --- | --- |
| 자격증 근거 라이브 조회 | `CAREERTUNER_CERT_DATA_GO_KR_SERVICE_KEY` 에 data.go.kr **디코딩(raw)** 키 주입. 미설정 시 라이브 조회 없이 안전 degrade(일정·등록 근거 미생성). q-net 은 **http 만** 지원(443 리스너 없음) — 업스트림 장애 시 '없음'이 아니라 **확인 불가**로 표시된다. 국가자격 **종류 판별(T/S)은 번들 오프라인 스냅샷**(resources/cert/, 연 단위 갱신)이 기본이라 Q-Net 장애와 무관하게 동작한다(`CAREERTUNER_CERT_CATALOG_SNAPSHOT_ENABLED=false` 로 네트워크 경로 강제 가능) |
