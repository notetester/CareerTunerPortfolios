...
/**
 * 면접 LLM 디스패처: Gemini 우선 → 실패/한도초과/미설정 시 OpenAI 폴백.
 *
 * <p>호출부({@link InterviewOpenAiClient})는 이 @Primary 게이트웨이만 주입받으므로,
 * provider 전략은 여기 한 곳에서만 결정된다.
 * <ul>
 *   <li>Gemini 키 있음 → Gemini 시도, BusinessException(요청 실패·한도초과 등) 발생 시 OpenAI 로 폴백</li>
 *   <li>Gemini 키 없음 → 곧장 OpenAI</li>
 *   <li>둘 다 없음 → 명확한 예외</li>
 * </ul>
 */
@Primary
...
public class FallbackInterviewLlmGateway implements InterviewLlmGateway {
    ...
    private final GeminiLlmGateway gemini;
    private final OpenAiLlmGateway openAi;

    public FallbackInterviewLlmGateway(GeminiLlmGateway gemini, OpenAiLlmGateway openAi) {
        this.gemini = gemini;
        this.openAi = openAi;
    }
    ...
    public Result complete(Request request) {
        if (gemini.available()) {
            try {
                ...
                if (openAi.available()) {
                    log.warn("Gemini 호출 실패 → OpenAI 폴백 ({}): {}", request.schemaName(), ex.getMessage());
        ...
        throw new BusinessException(ErrorCode.INTERNAL_ERROR,
                "LLM provider 가 설정되어 있지 않습니다. GEMINI_API_KEY 또는 OPENAI_API_KEY 를 설정하세요.");
    }
}
