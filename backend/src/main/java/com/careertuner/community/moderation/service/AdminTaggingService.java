...
/**
 * 관리자용 태깅 배치/단건 재실행 서비스.
 *
 * 배치는 별도 스레드에서 순차 실행 (moderationExecutor 큐 포화 방지).
 * tag() 내부가 Ollama 동기 호출이라, 한 건씩 처리하면 자연스럽게 속도 조절됨.
 */
@Service
public class AdminTaggingService {

    private static final Logger log = LoggerFactory.getLogger(AdminTaggingService.class);

    private final CommunityPostMapper postMapper;
    private final PostAiResultMapper aiResultMapper;
    private final PostModerationService moderationService;

    // 배치 상태 추적 (단일 인스턴스 기준, 동시 배치 실행 방지)
    ...
    public AdminTaggingService(CommunityPostMapper postMapper,
    ...
    public int countTargets(boolean force) {
        return postMapper.findPostIdsForTagging(force).size();
    }

    /**
     * 배치 태깅 시작. 즉시 반환, 내부에서 비동기 순차 처리.
     * @return 대상 건수
     */
    public int startBackfill(boolean force) {
        ...
        List<Long> postIds = postMapper.findPostIdsForTagging(force);
        if (postIds.isEmpty()) {
            return 0;
        }
    ...
    /**
     * 별도 스레드에서 순차 태깅. moderationExecutor가 아닌 기본 async 풀 사용.
     */
    @Async
    void runBackfillAsync(List<Long> postIds) {
        log.info("태깅 배치 시작: {}건", postIds.size());
        try {
            for (Long postId : postIds) {
                try {
                    ...
                    log.warn("배치 태깅 실패: postId={}", postId, e);
                }
            }
        } finally {
            batchRunning = false;
            log.info("태깅 배치 완료: total={}, completed={}, failed={}",
     ...
     * 단건 재태깅.
     */
    public void tagSingle(Long postId, boolean force) {
        if (!force) {
            PostAiResult existing = aiResultMapper.findByPostIdAndTaskType(postId, AiTaskType.TAG);
            if (existing != null && "COMPLETED".equals(existing.getStatus())) {
                throw new IllegalStateException(
                        "이미 태깅 완료된 게시글입니다 (postId=" + postId + "). 강제 재실행은 ?force=true");
            }
        }
        moderationService.tag(postId);
    }
