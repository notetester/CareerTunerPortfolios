...
/**
 * 사용자가 고른 OCR 모델로 실패한 공고 추출을 <b>동기 strict 재추출</b>한다(수동 재추출 정책).
 *
 * <p>배경 워커와 달리 사용자가 결과를 기다리는 동기 경로이고, 다음 정책을 강제한다:
 * <ul>
 ...
 *   <li>실패 시 기존 공고·분석을 보존하고 <b>초기 실행 프로필을 건드리지 않는다</b>(reopen/abandon 없음).</li>
...
public class JobPostingReextractionService {
    ...
    public ApplicationCaseExtractionResponse reextract(Long userId, Long applicationCaseId, String ocrProvider) {
        ...
        if (!EXTRACTION_STATUS_FAILED.equals(latest.getStatus())) {
            throw new BusinessException(ErrorCode.CONFLICT, "실패한 최신 공고 추출 작업만 재추출할 수 있습니다.");
        }
        ...
        Long failedJobPostingId = latest.getJobPostingId();
        ...
        JobPosting posting = jobPostingService.getJobPostingDomainForCase(userId, applicationCaseId, failedJobPostingId);

        ApplicationCaseExtraction extraction = insertAndClaim(userId, applicationCaseId, failedJobPostingId, sourceType, provider);
