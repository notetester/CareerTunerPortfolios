...
INSERT INTO application_case (user_id, company_name, job_title, posting_date, source_type, status, is_favorite)
 SELECT id, '카카오페이', '프론트엔드 개발자', '2026-07-01', 'TEXT', 'READY', 1 FROM users WHERE email = 'jiwon.kim@careertuner.dev';
INSERT INTO application_case (user_id, company_name, job_title, posting_date, source_type, status, is_favorite)
 SELECT id, '국민건강보험공단', '전산직', '2026-07-10', 'MANUAL', 'ANALYZING', 0 FROM users WHERE email = 'jiwon.kim@careertuner.dev';
INSERT INTO application_case (user_id, company_name, job_title, posting_date, source_type, status, is_favorite)
 SELECT id, '현대자동차', '생산관리', '2026-06-20', 'TEXT', 'DRAFT', 0 FROM users WHERE email = 'jiwon.kim@careertuner.dev';
INSERT INTO application_case (user_id, company_name, job_title, posting_date, source_type, status, is_favorite)
 SELECT id, 'CJ올리브영', '마케팅 기획', '2026-07-05', 'TEXT', 'READY', 1 FROM users WHERE email = 'seoyeon.lee@careertuner.dev';
INSERT INTO application_case (user_id, company_name, job_title, posting_date, source_type, status, is_favorite)
 SELECT id, '스타트업 A', '서비스 운영 매니저', NULL, 'MANUAL', 'DRAFT', 0 FROM users WHERE email = 'seoyeon.lee@careertuner.dev';
...
INSERT INTO job_analysis (application_case_id, employment_type, experience_level, required_skills, preferred_skills, duties, difficulty, summary)
 ...
 WHERE u.email = 'jiwon.kim@careertuner.dev' AND ac.company_name = '카카오페이' LIMIT 1;
...
INSERT INTO community_post (user_id, category, title, content, company_name, job_title, interview_type, difficulty, is_anonymous)
 SELECT id, 'INTERVIEW_REVIEW', '카카오페이 프론트엔드 1차 면접 후기', 'React 프로젝트의 문제 해결 경험을 깊게 물어봤습니다. 수치로 답하니 반응이 좋았어요.', '카카오페이', '프론트엔드 개발자', 'FIRST', 'NORMAL', 1
 FROM users WHERE email = 'jiwon.kim@careertuner.dev';
INSERT INTO community_post (user_id, category, title, content, interview_type, difficulty, is_anonymous)
 SELECT id, 'QNA', '마케팅 직무 자기소개 어떻게 시작하면 좋을까요', '퍼포먼스 마케팅에서 기획으로 전환하는 스토리를 어떻게 풀어야 할지 고민입니다.', NULL, 'EASY', 0
 FROM users WHERE email = 'seoyeon.lee@careertuner.dev';
