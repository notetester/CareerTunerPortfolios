...
화면·기술 시안은 다음 순서로 찾는다. 이 문서와 `desktop/` 런타임 소스가 현재 구현의 정본이다.

| 자산 | 용도 |
| --- | --- |
| [desktop-app-dotnet-concept.html](../docs/archive/2026-06/desktop-app-dotnet-concept.html) | C#/.NET 컨트롤 센터 초기 구상 보관본(현재 구현 아님) |
| [desktop-app-prototype.html](../docs/desktop-app-prototype.html) | 초기 인터랙티브 화면 시안 |
| [desktop-app-stack.html](../docs/desktop-app-stack.html) | C++/Qt 기술 스택 설명 |
| [desktop-app-v2-mockup.html](../docs/desktop-app-v2-mockup.html) | 현행 v2 셸의 시각 기준 |
| [mobile-app-v2-mockup.html](../docs/mobile-app-v2-mockup.html) | 모바일 이어받기·기기 연동 화면 비교 |

## 요구사항
- **Qt 6.8+** (Quick · Network · Widgets · **Multimedia**; 릴리즈/CI 검증 버전 6.11.1)
- **CMake 3.16+**
...
│  ├─ AiChargeCoordinator    AI 실행 전 차감 미리보기·actionKey 전달
│  ├─ VoiceRecorder          Qt Multimedia 마이크 녹음 (m4a)
│  ├─ CameraRecorder         Qt Multimedia 카메라·영상 녹화
...
│  ├─ CommunityClient        커뮤니티 글 목록·상세·작성·댓글·좋아요
│  ├─ PlannerClient          일정·리마인더 조회와 상태 변경
│  ├─ PlannerOverlayController 전역 일정 오버레이 상태 조정
│  ├─ AdClient               광고 슬롯·노출·클릭 연동
│  ├─ ConsentClient          AI·이력서·영상 동의 경계 연동
...
└─ qml/                      ── 화면(QML · CC Desktop 문법) ──
   ...
   ├─ BoardPage.qml          커뮤니티 게시판
   ├─ NotificationCenter.qml 알림 센터
   ├─ PlannerOverlay.qml     일정·리마인더 전역 오버레이
...
- [x] AI 실행 전 차감 미리보기와 서버 발급 `actionKey` 전달
- [x] 일정·리마인더 오버레이, 광고 슬롯, 커뮤니티 게시판 연동
- [x] AI·이력서·영상 기능 동의 경계와 카메라 영상 녹화
...
다중 구성 generator에서 Release를 빌드했다면 `ctest --test-dir build -C Release --output-on-failure`를 사용한다.

## 백엔드 연결

기본은 공개 AWS 통합 백엔드(`https://careertuner.example.com`). 설정 화면에서 로컬·Tailscale 주소로 바꿀 수 있고 QSettings 에 영속화된다
