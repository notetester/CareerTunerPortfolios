...
운영 모델 태그는 `careertuner-e-correction-3b:latest`를 기준으로 하며, 원문에 없는 경험을 만들어내지 않는 원문 보존형 개선 후보 생성기로 운용한다.
...
| 2 | 공통 베이스 모델 | **Qwen2.5-3B** (한국어 양호 + Apache 2.0). E 첨삭도 동일 베이스를 사용한다. |
| 3 | 베이스 통일 여부 | 통일을 기본값으로 하되, F 검열만 기존 gemma 자산을 유지한다. |
| 4 | 서빙 | Ollama (로컬·무료) |
| 5 | 학습 환경 | Colab 무료 → 막히면 Pro |
| 6 | 모델 구성 | 담당자별 1모델 |
| 7 | E 자체모델 범위 | Qwen2.5-3B 기반 첨삭 통합 1모델(24~27). Qwen3-8B/4B 예외는 사용하지 않는다. |
...
| E | 첨삭 통합(24~27) | 자기소개서 첨삭 | D 채점 ↔ E 첨삭 연계 흐름. Qwen2.5-3B 단일 모델과 원문 보존 검증을 운영안 기준으로 관리 |
...
- **베이스 통일 예외(#3·#7·#8)**: 베이스는 Qwen2.5-3B로 통일하고 E 첨삭도 동일 베이스를 사용한다. F의 검열(33)만 기존 gemma 자산을 유지하며, 예외 모델을 쓰는 담당자는 모델 카드에 사유, 라이선스, 서빙 사양, fallback 기준을 기록한다.
