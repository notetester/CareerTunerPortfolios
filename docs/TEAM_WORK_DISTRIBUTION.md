...
근거·일정·비용·리스크의 단일 기준 문서는 `docs/planning/자체LLM_팀_도입안.md`이며, 본 절은 팀 분담 관점의 확정 사항만 정리한다. 충돌 시 도입안 문서를 우선한다.
...
| 2 | 공통 베이스 모델 | **Qwen2.5-3B** (한국어 양호 + Apache 2.0) |
| 3 | 베이스 통일 여부 | 통일 (단 F 검열만 gemma 예외 — #8) |
| 4 | 서빙 | Ollama (로컬·무료) |
| 5 | 학습 환경 | Colab 무료 → 막히면 Pro |
| 6 | 모델 구성 | 담당자별 1모델 |
| 7 | E 자체모델 범위 | 첨삭 통합 1모델 (24~27 한 모델) |
...
| E | 첨삭 통합(24~27) | 자기소개서 첨삭 | D 채점 ↔ E 첨삭 연계 흐름 |
...
- **베이스 통일 예외(#3·#8)**: 베이스는 Qwen2.5-3B로 통일하되, F의 검열(33)만 기존 gemma 자산을 유지한다. 학습 베이스만 2종이고 서빙은 동일 Ollama이므로 노트북·서빙 절차는 공유된다.
