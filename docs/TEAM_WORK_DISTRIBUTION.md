...
모델명은 `careertuner-e-correction`을 기준으로 하며, 원문에 없는 경험을 만들어내지 않는 원문 보존형 개선 후보 생성기로 운용한다.

| 우선순위 | 모델/방식 | 용도 |
| --- | --- | --- |
| 1 | Qwen3-8B 계열 E LoRA 모델 | 문장 생성 품질과 변경 이유의 구체성이 필요한 주력 첨삭 모델 |
| 2 | Qwen3-4B-Instruct 계열 E LoRA 모델 | 개인 PC, 무료 서버, 저사양 시연 환경의 경량 자체 모델 |
| 3 | 팀 공통 Qwen2.5-3B E LoRA 모델 | 공통 파이프라인 검증과 빠른 콜드스타트 |
| 4 | 규칙 기반 첨삭 엔진 | 위험 표현, 길이, 키워드 누락, 허위 사실 여부 검사 |
| 5 | OpenAI 기반 첨삭 | 자체 모델이 반복 실패하거나 발표 시연 안정성이 필요한 최종 fallback |
...
## 13. 자체 LLM 파인튜닝 트랙 (공통 합의)

34개 AI 기능이 전부 외부 API에 의존하는 구조를 보완하기 위해, 6명이 동일 파이프라인(합성 데이터 증류 → Colab 무료 LoRA → 로컬 Ollama 서빙 → 외부 API 폴백)으로 각자 도메인의 자체 모델을 하나씩 만든다.
근거·일정·비용·리스크의 기준 문서는 `docs/planning/자체LLM_팀_도입안.md`이며, 담당별 모델 운영·검증·fallback·산출물 기준은
`docs/planning/담당별_자체LLM_운영안.md`를 따른다. 본 절은 팀 분담 관점의 확정 사항만 정리한다.

> **[일단결정]** 아래 공통 결정은 진행을 막지 않기 위한 잠정 확정안이다(D 단독 초안). 팀 회의에서 조정될 수 있으나, 확정 전까지는 이 값을 작업 기준으로 삼는다.

### 공통 결정 (도입안 7장 결정표 확정)

| # | 결정 항목 | 결론 |
| --- | --- | --- |
| 1 | 선생(데이터 생성) 모델 | Flash-Lite 단일 (채점 등 품질 부족 시 해당 task만 후속 상향) |
| 2 | 공통 베이스 모델 | **Qwen2.5-3B** (한국어 양호 + Apache 2.0). 단 E 첨삭은 품질 요구 때문에 Qwen3-8B/4B 예외 후보를 운영안에 둔다. |
| 3 | 베이스 통일 여부 | 통일을 기본값으로 하되, F 검열은 gemma 유지, E 첨삭은 Qwen3 계열 예외 후보 허용 |
| 4 | 서빙 | Ollama (로컬·무료) |
| 5 | 학습 환경 | Colab 무료 → 막히면 Pro |
| 6 | 모델 구성 | 담당자별 1모델 |
| 7 | E 자체모델 범위 | 첨삭 통합 1모델 (24~27 한 모델). 주력 후보는 Qwen3-8B, 저사양 fallback은 Qwen3-4B 또는 팀 공통 Qwen2.5-3B |
| 8 | F 검열 처리 | gemma 유지 (F 기존 구현 자산 재활용) |
| 9 | ai/common 승격 | 함 (F Ollama 서빙을 공통 승격, 팀장 승인 필수) |
| 10 | 합성 JSONL 보관 | repo 커밋 (양 많으면 git LFS) |

### 담당자별 자체 모델 매핑

각 담당자는 자기 도메인 AI 기능 중 1순위 task부터 자체 모델로 만든다. 콜드스타트는 추출·분류·요약 도메인(A·B·F)부터 검증하고, 생성·추론 도메인(C·D·E)은 외부 API 폴백을 두텁게 깔고 진행한다.

| 담당 | 자체 모델 도메인 | 1순위 task | 비고 |
| --- | --- | --- | --- |
| A | 이력서·스펙 추출/요약 | 기술스택·키워드 추출 | 추출형 → 콜드스타트 유리 |
| B | 공고 분석 추출 | 공고문 문장 분류와 필수/우대 조건 추출 | OCR 이후 텍스트 구조화 추출 → 콜드스타트 유리 |
| C | 커리어 전략·추천 | 적합도 분석 | 생성·추론 → 폴백 비중↑ |
| D | 가상 면접 | 답변 채점(22) | 심화: Planner까지 자체 모델(자율 에이전트 두뇌) |
| E | 첨삭 통합(24~27) | 자기소개서 첨삭 | D 채점 ↔ E 첨삭 연계 흐름. Qwen3-8B/4B 예외 후보와 원문 보존 검증을 운영안 기준으로 관리 |
| F | 커뮤니티 검열 분류(33) | 부적절 분류 | 구현 완료 → 파인튜닝 승격. **팀 공통 서빙 인프라의 출발점** |

### 공통 영역 영향 (11장 연계)

- **ai/common 승격(#9)**: F의 Ollama 로컬 서빙을 `backend/.../ai/common`으로 승격해 6명이 공유한다. `ai/common`은 팀장 Owner 공통 영역이므로 **팀장 승인 또는 팀 합의 후** 진행한다.
- **베이스 통일 예외(#3·#7·#8)**: 베이스는 Qwen2.5-3B로 통일하되, F의 검열(33)은 기존 gemma 자산을 유지하고, E 첨삭은 문장 생성 품질 때문에 Qwen3-8B/4B를 예외 후보로 둘 수 있다. 예외 모델을 쓰는 담당자는 모델 카드에 사유, 라이선스, 서빙 사양, fallback 기준을 기록한다.
- **합성 데이터 보관(#10)**: 도메인별 합성 JSONL은 repo에 커밋한다(용량이 크면 git LFS). 운영 DB에는 적재하지 않는다. OneDrive 등 개인 스토리지는 팀 공통 보관소로 쓰지 않는다.
