...
> 기준일: 2026-07-13
> 코드 기준점: `origin/dev` `dbf2514e6b523769a2385eb95e8da2d9b93627e9`
...
모델명이나 코드 주석만 “파인튜닝”이라고 부르고 학습 package가 없는 경우는 위 두 분류에 억지로 넣지 않고 **학습 주장·provenance 미검증**으로 별도 표시한다. 최신 F의 `careertuner-mod`가 이 사례다. 해당 자료가 복원되기 전에는 직접 학습 모델 수나 성과에 포함하지 않는다.
...
> “저희가 자체 모델이라고 부르는 것은 범용 LLM을 0부터 사전학습했다는 뜻이 아닙니다. 공개 또는 별도 사용 조건이 있는 base를 영역별로 검토해 채택했고, A/B/C/D/E의 일부 생성 모델에는 CareerTuner 데이터 기반 LoRA/QLoRA를 적용했습니다. F의 Qwen3·Gemma 4·Qwen2.5-VL·BGE-M3는 직접 파인튜닝하지 않고 자체 hosting provider, prompt, tool, RAG와 검증 계층으로 통합했습니다. 최신 F 코드에는 검열 전용 파인튜닝 모델이라고 명명한 `careertuner-mod`도 연결됐지만, base·학습 데이터·방식·adapter·평가·라이선스가 보존되지 않아 검증된 학습 자산으로 단정하지 않습니다. 직접 만든 핵심은 데이터 계약, 근거가 남은 adapter 학습, 평가·검증, merge·serving과 fallback까지의 전체 pipeline입니다. 상용 가능 여부는 모델별 LICENSE와 실제 serving digest를 따로 확인합니다.”
...
| F | `careertuner-mod` | 프로젝트 파인튜닝 주장, 근거 미보존 | 게시글·댓글 1차 tag 연결, 설치·학습·평가·digest 미검증 | 짧은 텍스트 검열 JSON |
| F | `gemma4` | 자체 호스팅 OSS | 코드 설정 확인, live digest·서빙 미검증 | 태깅·후기 추출·신고 참고 분류·관리자 지원 생성 |
...
| 설정 | 문제와 의도 | 근거 수준 |
| --- | --- | --- |
| r=16 | 표현력과 VRAM 절충 | **rank sweep 실측(§13.5.5): r16이 contract 최고(95.0%)** |
| alpha=32 | update scaling 2 | alpha 고정 sweep 미실행(rank와 함께 α=2r로만 변경) |
...
C 스크립트 주석은 D 설정을 재사용했다고 밝힌다. 원래는 "C에서 r=16이 최적이라고 찾았다"가 아니라 "공유 설정으로 학습을 완주하고 task 평가로 충분성을 봤다"가 정확했다. **다만 2026-07-13 rank sweep(§13.5.5)이 원본 데이터로 r=8/16/32를 신규 학습해, r16이 이 task·데이터에서 계약(contract) 균형점(95.0%로 최고)임을 사후 실측했다** — 단 r32가 grounding·loss는 더 낫다는 tradeoff도 함께 확인됐으므로 r16을 "전역 최적"으로 단정하지는 않는다.
...
변환은 `merge_and_unload` → HF safetensors/tokenizer → llama.cpp f16 GGUF → Q4_K_M → Ollama 등록 순서다(`scripts/merge_and_export.py:20-42`). F16 5.75GiB를 Q4 1.80GiB로 줄였다. **동일 golden set의 f16↔Q4 품질 비교는 2026-07-13에 실측했다(§13.5.4): Q4는 f16 대비 contract -6.7pp·CJK -3.3pp를 대가로 latency ~0.51×·GPU -4058MiB를 얻는 명시적 절충으로, quality-neutral이 아니다.**
...
512 tokens에서 JSON truncation이 났고 운영 여유를 1,280으로 뒀다. Properties는 1,024 미만을 부팅 오류로 막는다(`CareerAnalysisAiProviderProperties.java:24-106`). **2026-07-13 절단 곡선(§13.5.3) 정정: golden60에서 실패 0인 최소 cap은 768이었고 1,024가 반드시 필요하다는 근거는 재현되지 않았다 — 1,024/1,280은 미지 입력 여유 마진이다.** Temperature 0.2는 구조 변동을 낮추면서 소량 다양성을 남긴 값이었는데, **온도 sweep(§13.5.2)을 실측하니 0.0이 contract·CJK·자기일관성에서 0.2보다 낫고(단 E1은 소폭 나쁨) 0.7 이상에서 급격히 무너졌다. 0.0조차 Jaccard 0.68로 완전 결정적이지 않다(Ollama 서버측 비결정).**
...
Q4는 6.18GB→1.93GB 약 3.2배 축소됐지만 부분 정답 구간 오차를 더 키워 배포를 거부했다(`eval/LIVE_AB_RESULT.md:1-53`). 이 결정은 “양자화가 늘 좋다”는 가정 대신 task calibration을 본 사례다. `eval/README.md`의 20건 문구는 stale하고 실제 fixture는 60건이다. **참고: 2026-07-13 golden20 신규 실측(§13.5.6)은 f16 MAE 7.90 / Q4 8.15로 절대값이 위 표(판정단 기준 8.30/9.53)와 다른데, 이는 채점 기준(golden 라벨 vs 판정단)과 N(20 vs 60)이 달라서다 — 상충이 아니라 별개 fixture의 결과다.**
...
### 11.1 F의 검열 전용 모델 주장과 증거 경계

F는 주로 공개 가중치 모델을 자체 Ollama provider 경로에 연결하고 prompt·tool·RAG·schema·code guard·hosted fallback을 결합한 서비스 특화 AI다. Qwen3·Gemma 4·Qwen2.5-VL·BGE-M3에는 F 전용 adapter 근거가 없다. 다만 최신 코드 `fab886be`는 게시글·댓글 검열용 기본 tag `careertuner-mod`를 추가하며 이를 “검열 전용 파인튜닝 모델”이라고 부른다.

이 명칭만으로 학습 완료를 증명할 수는 없다. 저장소 전체에는 `careertuner-mod`의 base model·revision·LICENSE, dataset·split·teacher, full FT/LoRA/QLoRA 여부, rank·alpha·dropout·target module, optimizer·LR·epoch·batch·sequence·seed, trainer state·loss, adapter·merged weight·GGUF checksum, 실제 Modelfile, held-out evaluation이 없다. 원격 Ollama `/api/show`도 10초 안에 응답하지 않아 tag 존재·digest·quantization을 확인하지 못했다. 따라서 이 보고서는 **“파인튜닝으로 명명된 런타임 tag가 연결됐다”**는 사실과 **“재현 가능한 학습 자산으로 검증됐다”**는 주장을 분리한다.

| 역할 | 모델 | 프로젝트 적용 | 직접 학습 |
| --- | --- | --- | --- |
| 챗봇·인테이크·후속질문·요약·화행 | `qwen3:8b` | prompt, tool, memory, guard, fallback | 없음 |
| 게시글·댓글 텍스트 검열 | `careertuner-mod` | 짧은 JSON schema, threshold, review queue | 코드가 파인튜닝이라 명명, 학습 근거 없음 |
| tag·면접후기 추출·신고 참고 분류 | `gemma4` | 범용 structured JSON | 없음 |
...
근거: `application.yaml:332-351`, `CommunityAgentConfig.java:18-46`, `IntakeAgentConfig.java:27-48`, `SpeechActClassifier.java:23-95`. Temperature 0은 동일 라우팅, tool argument, 짧은 label, 자동 test 재현성을 위한 운영 선택이지만 공식 sampling과의 A/B 최적화는 없다.
...
### 11.3 `careertuner-mod` 텍스트 검열 전용 tag

`careertuner-mod`는 게시글·댓글의 `normal|abuse|spam|ad` 판정을 범용 Gemma에서 분리하기 위해 추가됐다. 코드 설명상 외부 Modelfile에 `num_predict=64`가 고정돼 짧은 판정 JSON에 맞고, 긴 tag·면접후기 JSON은 잘릴 수 있어 두 모델을 나눴다. 이는 **서빙 책임을 분리한 이유**는 설명하지만 **어떤 base를 어떤 데이터와 알고리즘으로 파인튜닝했는지**는 설명하지 않는다.

현재 runtime 경로:

```text
게시글·댓글 검열: careertuner-mod → Claude Haiku → OpenAI → Mock
신고 참고 분류:   gemma4         → Claude Haiku → OpenAI → Mock
자동 tag·후기 추출: gemma4       → Claude Haiku → OpenAI → Mock
```

근거: `OllamaProperties.java:22-28`, `application.yaml:311-316`, `ModerationLlmGateway.java:67-113`, `PostModerationService.java:247-278,459-460,509,614`.

| 구분 | 현재 확인값 | 증거 수준 |
| --- | --- | --- |
| runtime tag | `careertuner-mod` | Git 설정·호출 코드 확인 |
| 역할 | 게시글·댓글 텍스트 검열 1차 tier | Git 호출 경로 확인 |
| temperature / context | `0` / `8192` | request body 확인 |
| stream / think | `false` / `false` | request body 확인 |
| retry | 최초+2회, backoff 1s/3s | 공통 Ollama client 확인 |
| `num_predict` | `64`라고 코드 주석·PR에 기록 | 실제 Modelfile 미추적 |
| base·parameter 수·license | 알 수 없음 | 증거 없음 |
| full FT/LoRA/QLoRA·rank·alpha | 알 수 없음 | 증거 없음 |
| 데이터 수·split·teacher·PII | 알 수 없음 | 증거 없음 |
| LR·optimizer·epoch·batch·sequence·seed | 알 수 없음 | 증거 없음 |
| loss·FP/FN·precision/recall/F1·calibration | 알 수 없음 | model 전용 artifact 없음 |
| adapter·GGUF·Modelfile·checksum·digest | 알 수 없음 | artifact·live 응답 없음 |

공통 요청은 temperature0, think/stream false, `num_ctx=8192`, JSON Schema, connect10s/read60s를 사용하고 5xx·connection에 총 3회 시도한다(`OllamaClient.java:84-192`). 입력은 최대 8,000자이며 auto hide 기본 confidence≥.80, admin 조절 .50~.95, low-confidence toxic은 review queue로 보낸다. Mock·confidence null은 정상 판정이 아니라 `UNMODERATED`다. 이 모델에는 RAG가 없다.

독립 검증을 위해서는 base repo/revision/LICENSE, 학습 JSONL hash와 split, 데이터 출처·동의·PII 처리, 학습 방식과 모든 hyperparameter, trainer state, adapter/merged/GGUF checksum, 실제 Modelfile, 동일 blind set의 Gemma/base/candidate confusion matrix·ECE·latency를 복원해야 한다. 그 전에는 `num_predict=64` 외 학습 parameter나 “Gemma보다 검열 정확도가 높다”는 결과를 면접에서 말하면 안 된다.

### 11.4 Gemma 4 범용 구조화·운영지원 생성
...
그러나 프로젝트는 `gemma4:e4b`나 digest가 아닌 `gemma4` shorthand만 쓴다(`application.yaml:311-316`). Registry latest 또는 private tag가 바뀌면 실제 weight도 바뀐다. 조사 시 원격 Ollama `/api/show`·`/api/tags`가 timeout됐고 local에도 F model이 없었으며 2026-07-06 GPU artifact에는 gemma4가 아직 pull되지 않았다고 적혔다. **코드 설정은 확인됐지만 실제 운영 digest·서빙 상태는 미검증**이다.

Gemma의 현재 업무는 주 chatbot이나 게시글·댓글 1차 검열이 아니라 다음이다.

- 신고 참고용 toxic/abuse/spam/ad 분류
- 자동 tag
...
신고 참고 분류·tag·후기 추출은 temperature0, think false, `num_ctx=8192`, stream false, task JSON Schema, connect10s/read60s, 총 3회, backoff1s/3s다. 4xx는 즉시 실패하고 5xx·connection만 retry한다(`backend/src/main/java/com/careertuner/community/moderation/client/OllamaClient.java:39-192`).
...
첫 schema는 게시글·댓글에서는 `careertuner-mod`, 신고 참고 분류에서는 Gemma가 공유한다. 두 번째는 Gemma tag 경로다.
...
게시글·댓글 text moderation 체인은 `careertuner-mod`→Claude→OpenAI→Mock이고 신고 참고 분류는 Gemma4→Claude→OpenAI→Mock이다. Mock `toxic=false`만 보고 정상 처리하지 않는 것이 중요하다.

### 11.5 Qwen2.5-VL 7B 이미지 검열
...
### 11.8 왜 대부분의 F 모델은 별도 LoRA가 없는가

- FAQ·policy는 검색으로 최신성을 유지하는 편이 낫다.
- Intake ID는 모델 학습보다 server ownership 검증이 안전하다.
- `careertuner-mod`는 파인튜닝 모델이라고 명명됐지만 학습 방식·data·평가가 없어 LoRA인지조차 확인할 수 없다. 좁은 검열 분류에 tuning을 집중한 의도는 합리적일 수 있으나 증거 package가 먼저다.
- Vision tuning에는 개인정보·license가 정리된 image dataset이 필요하다.
- Retrieval tuning에는 positive pair와 hard negative가 필요하다.

Qwen3·Gemma·Qwen2.5-VL·BGE-M3에 대해서는 “base가 이미 최고라 tuning이 필요 없다”가 아니라 **튜닝을 정당화할 data·evaluation이 부족해 reversible prompt/RAG/guard를 먼저 택했다**가 정확하다. `careertuner-mod`는 반대로 tuning 주장은 생겼지만 근거가 빠진 사례이므로, 다른 F 모델의 미튜닝 이유와 섞어 설명하면 안 된다.

### 11.9 과거 audit와 현재 상태
...
### 11.10 저장소에서 확인한 평가 기록·test 자산과 공백
...
# careertuner-mod는 tracked Modelfile/pull source가 없어 기존 Ollama instance의 존재만 확인 가능
Invoke-RestMethod -Uri http://localhost:11434/api/show -Method Post `
  -ContentType application/json -Body '{"model":"careertuner-mod"}'

$env:AI_OLLAMA_BASE_URL='http://localhost:11434'
$env:AI_AGENT_MODEL='qwen3:8b'
$env:AI_OLLAMA_MODEL='gemma4:e4b'
$env:AI_OLLAMA_MODERATION_MODEL='careertuner-mod'
$env:AI_OLLAMA_VISION_MODEL='qwen2.5vl:7b'
$env:AI_CHATBOT_EMBEDDING_MODEL='bge-m3'
```

`careertuner-mod`는 `ollama create` recipe·Modelfile·base artifact가 추적되지 않아 새 clone에서 생성할 수 없다. 위 명령은 기존 instance 진단일 뿐 재현 명령이 아니다. Model tag뿐 아니라 `/api/show` digest·size·capabilities를 release manifest에 저장해야 한다. Model 변경 시 FAQ·community 전체를 재embedding하고 row에 embedding model version을 기록해야 stale vector를 식별할 수 있다.

### 11.12 F에서 과장하면 안 되는 것

- F가 Qwen/Gemma/BGE를 처음부터 학습했다.
- `careertuner-mod`가 LoRA/QLoRA이고 rank·alpha가 얼마라고 단정한다.
- `careertuner-mod`의 파인튜닝·라이선스·품질이 검증 완료됐다.
- 반대로 최신 F에는 파인튜닝으로 명명된 model tag가 전혀 없다고 말한다.
...
- ~~r 8/16/32 sweep~~ → **2026-07-13 C에서 실측(§13.5.5)**
- alpha independent sweep (여전히 미실행 — rank와 함께 α=2r로만 변경)
- attention-only vs all-linear (여전히 미실행)
- full fine-tuning baseline (여전히 미실행)
- 동일 base의 prompt-only vs LoRA vs RAG-only 전체 교차 (부분만 — §8.11)
- ~~f16 vs Q4_K_M 동일 golden 품질 비교~~ → **2026-07-13 C·D에서 실측(§13.5.4·§13.5.6)**

따라서 parameter 설명은 "왜 이 값이 공학적으로 합리적인가"와 "실제로 이 값이 최적임을 실험했는가"를 분리해야 한다. 위 취소선 항목은 후자(실측)까지 확보했고, 나머지는 아직 전자(공학적 합리성)에 머문다. C 외 영역(B·F)은 재현 자산이 로컬에 없어 이번 캠페인에서도 수치를 만들지 못했다(§13.5.7).
...
### 13.5 2026-07-13 정량 파라미터 실험 캠페인 (신규 실측)

§13.3이 "수행하지 않았다"고 정직하게 표시한 ablation 중 재현 자산이 로컬에 있는 것을 실제로 돌렸다. 아래 수치는 전부 **2026-07-13에 RTX 4090으로 새로 측정한 값**이며, 과거 리포트 수치나 추정을 재사용하지 않았다. GPU 학습·추론은 항상 모델 하나만 상주시켰고(모델 전환 전 `ollama stop`+VRAM 해제 barrier 확인), 독립 역집계 검증기(`validate_campaign.py`)가 raw에서 집계를 재계산해 `pass=8/fail=0`을 확인했다. provenance는 golden set·평가기·학습데이터·각 어댑터/GGUF의 SHA-256으로 고정했다.

- 공통 C 자산: golden `golden_fit_cases.jsonl` N=60(SHA `02d760c2…`), 평가 정의 `eval_fit_model.py`(SHA `f984c28a…`), 원본 학습 train375(SHA `0bdda856…`)/val41(SHA `ea25a2bb…`).
- 지표 정의는 §8.8~8.10과 동일 평가기를 그대로 사용한다(contract=전체 성공 판정, E1=grounding 위반율↓, CJK=중일 문자 누출율↓).

#### 13.5.1 요약 — 기존값 → 방치 시 문제 → 변경 → 정량 결과

| 대상 | 기존값과 관측 문제 | 변경 | 2026-07-13 4090 정량 결과 | 판단 |
| --- | --- | --- | --- | --- |
| C temperature | 0.2: contract 85.8%, CJK 7.5%, 자기일관성 Jaccard 0.3139 | 0.0 | contract 87.5%(+1.7pp), CJK 5.0%(-2.5pp), Jaccard 0.6831(+0.3692); 단 E1 +0.8pp, latency +29ms | 0.0을 후보로 삼되 transport 오류 없는 clean repeat 필요 |
| C output cap | 512: truncation·parse fail 각 43.33% | 768 | 둘 다 0%(-43.33pp), N=120 | golden60 최소 통과 cap은 **768**; 미지 입력 여유로 production은 1024 유지 가능 |
| C serving format | Q4: contract 88.3%, CJK 5.0%, 2658.9ms, GPU 3357MiB | f16 | contract +6.7pp, CJK -3.3pp; 대신 latency 1.97×, GPU +4058MiB, E1 +1.6pp | Q4는 quality-neutral이 아니라 **품질↔속도/VRAM 절충** |
| C LoRA rank | r8: eval loss 0.54208, contract 93.3%, E1 23.3% | r16 | eval loss 0.51576, contract **95.0%**, E1 18.3%; params 2배, peak +19MiB | r16이 이번 one-seed **contract 균형점** |
| C LoRA rank | r16: E1 18.3%, eval loss 0.51576 | r32 | E1 **6.7%**(-11.6pp), loss 0.50030; 대신 contract -1.7pp, params 2배, latency +217.9ms | E1 우선 후보, 추가 seed 필요 |
| D serving format | f16: MAE 7.90, 1365ms, GPU 7106MiB | Q4_K_M | MAE 8.15(+0.25), latency -48.8%, GPU -4058MiB, ±10 agreement 95% | 제안 gate PASS; D 오너 live A/B 비준 필요 |

#### 13.5.2 C temperature sweep (Q4, golden60, 온도별 repeat2=120회)

| temperature | parse | contract | E1 ↓ | CJK ↓ | 자기일관성 Jaccard ↑ | field exact ↑ | latency avg/p95 ms |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0.0 | 99.2% | 87.5% | 20.0% | 5.0% | 0.6831 | 28.81% | 2704.9 / 3263.9 |
| 0.2 (현행) | 99.2% | 85.8% | 19.2% | 7.5% | 0.3139 | 0% | 2675.9 / 3290.7 |
| 0.5 | 100% | 84.2% | 20.8% | 10.0% | 0.2312 | 0.33% | 2738.7 / 3314.9 |
| 0.7 | 97.5% | 82.5% | 27.5% | 8.3% | 0.1671 | 0% | 2765.5 / 3360.0 |
| 1.0 | 95.8% | 59.2% | 15.0% | 24.2% | 0.1448 | 0% | 2864.0 / 3577.7 |

온도를 올리면 contract·parse·자기일관성·CJK가 단조 악화하고 0.7 이상에서 급격히 무너진다(1.0의 contract 59.2%, CJK 24.2%). 0.0은 0.2보다 contract·CJK·일관성이 낫지만 E1이 소폭 나쁘고, 특히 **0.0에서도 Jaccard가 1.0이 아니어서(0.6831)** "완전 결정적 출력"으로 표현할 수 없다(Ollama 서버측 비결정성). 즉 현행 0.2는 구조 안정성과 소량 다양성의 무난한 값이고, 결정성이 중요하면 0.0이 후보다. **한계:** 0.2·1.0에서 각 1회 HTTP 500(peg-native 포맷 오류)이 120 분모에 남아 있어 이 실행은 `feasible:false`로 표기했다(실패 행을 숨기지 않고 포함).

#### 13.5.3 C output cap(num_predict) 절단 곡선 (Q4, temp 0.2, cap별 120회)

| num_predict | truncation | parse fail | 출력 token avg/p95 | latency avg/p95 ms |
| ---: | ---: | ---: | ---: | ---: |
| 256 | (1회 HTTP500로 미집계) | 120/120 | null | 1455.4 / 1475.4 |
| 512 | 43.33% | 43.33% | 478.3 / 512 | 2574.1 / 2763.5 |
| 768 | 0% | 0% | 503.3 / 618 | 2701.8 / 3272.6 |
| 1024 | 0% | 0% | 502.7 / 621 | 2701.7 / 3299.9 |
| 1280 (현행) | 0% | 0% | 495.5 / 615 | 2668.0 / 3272.9 |

512→768에서 truncation과 parse 실패가 43.33%→0%로 동시에 사라진다. **정정:** §8.7은 "512 truncation, 1,024 통과"라고 적었으나, 이번 golden60에서 실패 0인 최소 cap은 **768**이었고 1,024가 반드시 필요하다는 근거는 재현되지 않았다. 768·1024·1280의 작은 token/latency 차이는 확률적 실행 차라 cap 효과로 단정하지 않으며, production이 1024를 유지하는 것은 미지 입력 여유(margin)로 정당화한다.

#### 13.5.4 C f16 대 Q4_K_M 동일 golden 품질 (동일 보존 lineage, golden60, temp 0, repeat1)

| format | contract | E1 ↓ | CJK ↓ | latency avg/p95 ms | GPU total | Ollama size |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| f16 | 95.0% | 18.3% | 1.7% | 5243.2 / 6391.1 | 7415 MiB | 6.4 GB |
| Q4_K_M | 88.3% | 16.7% | 5.0% | 2658.9 / 3166.5 | 3357 MiB | 2.2 GB |

§8.7·§13.3이 "f16↔Q4 비교 없음"으로 남긴 공백을 채웠다. Q4는 f16 대비 **contract -6.7pp, CJK -3.3pp**를 대가로 **latency ~0.51×, GPU -4058MiB**를 얻는다(E1은 오히려 -1.6pp). D 면접에서도 Q4가 MAE를 소폭 악화시킨 것(§13.5.6)과 방향이 같다. 즉 C의 Q4 채택은 "무해한 축소"가 아니라 자원/응답속도를 위해 계약 품질을 일부 내준 **명시적 절충**이다. repeat1이므로 작은 차이를 확정 일반화로 쓰지 않는다.

#### 13.5.5 C LoRA rank sweep (원본 mixed416 신규 학습, seed42, 3ep, alpha=2r, golden60 temp0)

디스크에서 보존된 원본 train375/val41을 찾아 manifest SHA와 일치함을 확인하고 재생성 없이 사용했다. 각 rank를 이번에 새로 학습→CPU merge→f16 GGUF→Ollama 평가했다.

| r / α | trainable params | 4090 peak | train runtime | train / eval loss | contract | E1 ↓ | eval latency avg/p95 ms |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 8 / 16 | 14,966,784 | 22,527 MiB | 504.8s | 0.70437 / 0.54208 | 93.3% | 23.3% | 5254.7 / 6269.3 |
| 16 / 32 (현행) | 29,933,568 | 22,546 MiB | 499.1s | 0.62870 / **0.51576** | **95.0%** | 18.3% | 5257.4 / 6067.8 |
| 32 / 64 | 59,867,136 | 22,543 MiB | 506.5s | 0.57000 / 0.50030 | 93.3% | **6.7%** | 5475.3 / 6517.3 |

§8.3·§13.3이 "C rank sweep 없음"으로 남긴 것을 실측으로 대체한다. 핵심:
- **재현 검증:** 재학습한 r16의 eval loss 0.51576이 프로덕션 보고값 0.51546(§8.7)과 사실상 일치 → 하니스·데이터가 원본임을 입증.
- **contract는 r16에서 최고(95.0%)** — r8(93.3%)·r32(93.3%)보다 높다. rank를 키우면 eval loss는 단조 감소(0.542→0.516→0.500)하고 E1 grounding도 개선(23.3%→18.3%→6.7%)되지만, **계약 규율은 r16이 정점**이다. 즉 r16은 "공유 설정 재사용"이 아니라 이 task·데이터에서 계약 균형점이라는 정량 근거가 생겼다.
- r32는 params 2배·latency +217.9ms를 감수하면 E1을 크게 낮추는(6.7%) 후보다. one-seed 결과이므로 r32의 E1 우위는 추가 seed/repeat로 확인해야 하며 "최적 확정"이 아니다. 선택 항목이던 alpha 고정 sweep과 attention-only 대 +MLP는 이번에 실행하지 않았다(핵심 rank 3조건 우선).

#### 13.5.6 D 면접 f16 대 Q4_K_M (golden20, temp0, repeat2=40회씩)

| format | MAE vs golden ↓ | mean score | ±10 agreement | latency avg/p95 ms | GPU total | size |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| f16 | 7.90 | 55.30 | — | 1365.3 / 2275.7 | 7106 MiB | 6.4 GB |
| Q4_K_M | 8.15 | 55.95 | 95% | 699.6 / 1176.8 | 3048 MiB | 2.2 GB |

paired N=20에서 평균 절대 score 차 2.35, ±10 일치 95%, 10점 초과 차이는 1건(+25). Q4는 MAE +0.25 대가로 latency -48.8%·GPU -4058MiB를 얻는다. **주의(불일치 정직 표기):** §9.5·§13.2의 기존 D 수치는 60-케이스 dual-judge 패널 기준 **MAE 8.30→9.53**이고, 이번 실측은 golden20 기준 **7.90→8.15**로 규모·채점셋·시점이 달라 절대값이 다르다. 두 측정은 상충이 아니라 서로 다른 fixture의 별개 결과이며, 어느 쪽도 D 오너의 live A/B 비준 전까지 제품 승인 임계값이 아니다.

#### 13.5.7 실행했으나 재현 자산이 부족해 수치를 만들지 않은 영역 (정직 표기)

- **E 첨삭:** `careertuner-e-correction-3b`의 f16·Q4를 unseen24로 실제 실행했으나 두 프로세스 모두 rc=1로 중도 종료(f16=peg-native HTTP500, Q4=JSON decode 오류)해 비교 report JSONL을 만들지 못했다. 이는 f16/Q4 우열이 아니라 **현재 grammar/repair 경로가 전체 fixture를 완주하지 못한다는 결과**이며 `feasible:false`로 남겼다(불완전 조건 비교 금지).
- **B 공고:** `r1`/`r2` tag는 있으나 공통 OCR 회귀셋·B benchmark·자동 품질 verdict 계약이 로컬에 없어 tag 존재만으로 수치를 만들지 않았다.
- **F 라우팅/검열:** 독립 golden runner와 필요한 로컬 tag(`gemma4`, `qwen3:8b`)가 없어 mock/prompt 예시를 실험으로 가장하지 않았다.

이 세 공백은 §14의 "과장 금지"와 같은 원칙이다 — 자산이 없으면 수치를 만들지 않는다. 재현 캠페인 원자료는 제품 저장소가 아닌 4090 `ct-param-exp/`(results·raw·`validation_report.json`)에 보존했다.
...
| P0 | `careertuner-mod` base·license·학습 provenance 부재 | 상용 사용·재현·품질 주장 불가 | base/LICENSE, data, trainer state, Modelfile, weight checksum 정본화 |
...
| P1 | F runtime model provenance | qwen3/gemma4/qwen2.5vl/bge-m3/careertuner-mod의 운영 base·quantization·digest 미확인 | versioned tag와 `/api/show` digest 정본화 |
| P1 | `careertuner-mod` 전용 회귀평가 부재 | 검열 FP/FN·confidence·fallback 비용 불명 | blind confusion matrix·ECE·latency·failure test |
...
- “모든 AI가 파인튜닝됐다.” — OCR, STT, embedding과 F의 Qwen/Gemma/VL/BGE는 통합 모델이며 `careertuner-mod`는 tuning 주장만 있고 근거가 없다.
...
| F 범용·검색 | prompt/schema/retrieval fixture | 별도 파인튜닝 없음 | Ollama pull/alias manifest | chat/tool/RAG/moderation test |
| F `careertuner-mod` | 미보존 | 방식·parameter 미보존 | Modelfile·artifact 미보존 | runtime tag 연결만 확인, model 평가 미보존 |
...
#### Q36. `careertuner-mod`는 어떤 모델을 어떻게 파인튜닝했습니까?

현재 근거만으로는 답할 수 없다. 코드와 PR은 검열 전용 파인튜닝 tag라고 설명하고 runtime은 temperature0·context8192로 호출하지만, base·LICENSE·data·학습 방식·LoRA parameter·loss·평가·Modelfile·digest가 없다. 따라서 “모른다”가 냉정한 답이며, 이 자료를 복원하기 전에는 직접 학습 성과나 Gemma 대비 우위를 발표하지 않는다. 대신 게시글·댓글만 이 tag로 분리하고 실패하면 Claude→OpenAI→Mock으로 이동하는 현재 서빙 구조는 코드로 증명할 수 있다.
...
- F `careertuner-mod` 현재 연결: `backend/src/main/java/com/careertuner/community/moderation/config/OllamaProperties.java`, `backend/src/main/java/com/careertuner/community/moderation/client/OllamaClient.java`, `backend/src/main/java/com/careertuner/community/moderation/client/ModerationLlmGateway.java`, `backend/src/main/java/com/careertuner/community/moderation/service/PostModerationService.java`, `backend/src/main/resources/application.yaml`; 학습 source·artifact·Modelfile은 없음
...
- F는 파인튜닝되지 않은 OSS 모델을 tool·retrieval·scope·moderation 정책과 결합해 운영하며, 새 `careertuner-mod`는 검열 전용 파인튜닝 tag로 연결됐지만 학습 provenance를 복원하기 전에는 검증된 자체 학습 성과로 계산하지 않는다.
