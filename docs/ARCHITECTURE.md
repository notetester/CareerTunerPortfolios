...
> **현재성 기준:** `origin/dev` `23bb4d22`를 2026-07-14에 재확인했다. 이 문서는 시스템 경계와
> 목표 구조의 정본이며, 실행 명령·환경변수·구현 API의 세부 상태는 각 모듈 `README.md`를 우선한다.
...
CareerTuner/                IntelliJ로 이 루트를 연다
 ...
 ├─ frontend/               React 19 + Vite + TS + Tailwind  (웹/PWA/Capacitor 공통 UI, :5173)
 ├─ desktop/                Qt/QML 데스크톱 앱 셸
 ├─ ml/                     자체 LLM 파인튜닝·평가 실험 산출물
 ├─ docs/                   기획·아키텍처 문서
 └─ frontend/android/       추적 중인 Capacitor Android 프로젝트
```

Capacitor iOS 의존성과 동기화 스크립트는 준비되어 있으나, iOS 네이티브 프로젝트 생성·빌드는 macOS에서
수행한다. 플랫폼별 실제 빌드 상태는 [`../frontend/MOBILE_BUILD.md`](../frontend/MOBILE_BUILD.md)를 따른다.
...
| 프런트엔드 | React 19, Vite 8, TypeScript 7, Tailwind CSS v4, shadcn/ui, react-router 8 |
| AI/ML | OpenAI Responses API, Ollama 로컬 LLM, LoRA/QLoRA 자체 모델 실험, JSON schema 검증/fallback |
| 모바일 | 반응형 웹 + PWA + Capacitor 8(Android 추적, iOS 빌드 절차 준비) |
| 데스크톱 | C++ / Qt 6.8+ / QML 네이티브 클라이언트. Qt Network REST·SSE, Qt Multimedia 녹음·촬영, 시스템 브라우저 웹 이어하기 |
...
**AI 모듈**은 공통 인프라(`ai/common`)와 도메인별 AI 서비스를 분리한다. `ai/prompt`는 현재
`package-info.java`만 있는 향후 공통 프롬프트 엔진의 예약 경계이며, 실행 중인 prompt builder·catalog·validator는
각 기능 도메인에 있다. 이렇게 해야 여러 담당자가 같은 `ai` 파일을 동시에 수정하는 충돌을 줄일 수 있다.
각 도메인은 자기 프롬프트 계약과 검증·폴백을 관리하고 토큰·크레딧 사용량을 기록한다
([제품 기획서의 AI 기능 설계](planning/기획.md#9-ai-기능-설계)).
팀 자체 LLM은 외부 API를 즉시 대체하는 단일 엔진이 아니라, 담당 도메인의 검증기와 폴백 체인 안에서
선택적으로 쓰는 provider다. 현재 A 프로필은 선택적 LoRA와 로컬 이력서 구조화, B는 Ollama 공고·기업 분석,
C는 선택적 커리어 전략 3B, D는 선택적 면접 생성·채점 OSS, E는 버전 고정 첨삭 3B, F는 Ollama
커뮤니티 검열·챗봇을 각각 런타임에 연결해 두었다. 자체 모델이 비활성·미설정·실패하면 도메인별 정책에 따라
Claude/OpenAI/규칙 또는 mock으로 강등하며, 모든 도메인이 같은 순서를 강제하지 않는다. 실제 기본값과 필요한
환경변수는 [`../backend/README.md`](../backend/README.md)의 A~F 표를, 목표 운영·검증 기준은
[`담당별 자체 LLM 운영안`](planning/담당별_자체LLM_운영안.md)을 따른다. 공통 `ai/common`, 공통 로그 구조,
...
기업 분석의 웹 근거 수집은 **외부 검색 API(네이버 검색)** 를 시스템 경계로 둔다. 이 경로는
`careertuner.company-websearch.enabled` 플래그로 게이트하며 현재 커밋 기본값은 **활성(ON)** 이다. OFF 상태에서는
검색 호출·검색 캐시 사용·WEB evidence 생성을 하지 않고 기존 공고 기반 분석 경로를 유지한다. ON이어도
자격증명이 없거나 검색이 실패하면 공고 기반 분석으로 안전하게 강등한다. 정상 연결 시에는
...
**의존 그래프대로 병렬** 호출해 6개 도메인(프로필·공고·적합도·자소서·면접·커뮤니티) 지원 준비 전체를 자동화한다.
각 핸들러는 담당 도메인의 실제 provider·검증·폴백 계약을 사용하며, 실행할 수 없는 단계는 명시적 mock/skip
상태로 남겨 전체 계획을 완주한다. 진행 상황은 SSE로 스트리밍한다.
...
ncs_classification                         공개 NCS 세분류·능력단위 계층 스냅샷
certificate ─1:N─ certificate_exam_schedule  공개 자격증 설명·시험일정 스냅샷
```
...
- 모든 엔드포인트는 `/api/**`.
- 일반 JSON REST 응답은 `ApiResponse<T>` (`success`, `code`, `message`, `data`). OAuth redirect, SSE,
  파일·이미지·ICS·JSONL download처럼 wire format 자체가 계약인 endpoint는 명시적 예외다.
...
GET  /api/catalog/ncs?q=
GET  /api/catalog/ncs/{id}
GET  /api/catalog/certificates?q=&type=
GET  /api/catalog/certificates/{id}
```
...
`/api/admin/**`는 먼저 인증된 `ADMIN` 또는 `SUPER_ADMIN` 역할을 요구한다. `SUPER_ADMIN`은 복구와
권한 운영을 위해 세부 권한 검사를 우회하고, 일반 `ADMIN`은 컨트롤러에 선언된 정본 권한 중 실제로
부여된 권한이 있어야 한다. 정본 권한은 아래 도메인의 `READ`, `CREATE`, `UPDATE`, `DELETE` 조합과
변조할 수 없는 `AUDIT_READ`다.

```text
USER / SECURITY / BILLING / CONTENT / AI / POLICY / ADMIN_PERMISSION
```

`ADMIN_PERMISSION_*`는 최고 관리자만 다룬다. 일반 관리자용 권한 템플릿은 기본적으로 조회·생성·수정을
묶고 삭제는 개별 부여하며, 실제 허용 여부는 직접 권한과 활성 그룹 권한의 합집합으로 계산한다.
프런트 라우트와 메뉴는 같은 정본 코드를 사용하지만 표시 제어일 뿐이며 서버 인터셉터가 최종 경계다.

공지, FAQ, 정책, 법적 문서·조항, 운영 메모, 챗봇 대화, 광고, 면접 지식처럼 관리자가 삭제할 수 있는
운영 데이터는 `deleted_at`을 기록하고 모든 관리자·사용자·공개 조회에서 제외한다. 유일 키를 가진 관계
데이터는 재등록할 때 새 중복 행을 만들지 않고 기존 행의 `deleted_at`을 `NULL`로 복원한다. refresh token,
MFA 백업 코드처럼 폐기가 보안 동작 자체인 자격 증명은 물리 폐기를 허용한다.

## 7. 프런트엔드 아키텍처

현재 프런트엔드는 사용자·관리자 기능을 함께 제공하는 반응형 SPA이며, 인증·지원 건·분석·면접·첨삭·결제·
커뮤니티·관리자 API가 기능 모듈에 연결되어 있다. PWA 서비스 워커와 Capacitor 런타임도 같은 코드베이스를
사용한다. 초기 Figma Make 화면은 디자인 출발점일 뿐 현재 구현 상태의 정본이 아니다. 앱 전역
라우트·레이아웃·API 기반은 `app/`에 두고, 기능 전용 API·hook·type은 각 기능 폴더 안에 둔다.
...
## 8. 모바일 / PWA 구현 상태와 운영 원칙

1. **완료:** 반응형 웹과 `vite-plugin-pwa` 기반 manifest·서비스 워커·아이콘
2. **완료:** Capacitor 8 공통 런타임, Android 프로젝트, OAuth/deep link, 카메라·햅틱·푸시 플러그인
3. **준비됨:** iOS 의존성·동기화·Associated Domains 스크립트. 네이티브 프로젝트 생성과 서명 빌드는 macOS 필요

재현 명령과 mock/live APK 구분은 [`../frontend/MOBILE_BUILD.md`](../frontend/MOBILE_BUILD.md)를 정본으로 한다.
