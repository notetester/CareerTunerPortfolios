...
> **보관 문서:** 2026-06 설계와 당시 구현 상태가 한 파일에 누적된 스냅샷이다. 현재 계약은 [`../../AI_ORCHESTRATOR.md`](../../AI_ORCHESTRATOR.md)를 따른다.
...
| Ollama 서버 | 원격 4090 private tailnet endpoint | `AI_OLLAMA_BASE_URL` |
...
### 11.5 앱 온보딩 퍼널 (마누스 레퍼런스)

앱 진입 순서. 부품(로그인·결제·알림·검색창)은 우리도 거의 다 있고, **순서로 꿰는 게 핵심**이다.

| 순서 | 화면 | 우리 현황 |
| --- | --- | --- |
| 1 | 환영 + 로그인(소셜/이메일) + 약관 동의 | 로그인 ✅(이메일). 소셜 로그인은 확인/검토 |
| 2 | 구독 제안 — **닫기(X)로 스킵 = 무료티어** | 결제·크레딧 ✅ `features/billing`(토스·구독). 크레딧 = `ai_usage_log.credit_used` |
| 3 | 알림 권한 요청 | 푸시 ✅ (VAPID·FCM, C 구현) |
| 4 | 메인 = 검색창("무엇을 도와드릴까요?") | `AutoPrepLauncher`를 첫 화면으로 끌어올리면 마누스 메인과 동일 |

> **이 프로젝트는 포트폴리오/시연용 — 스토어 미출시.** 따라서 스토어 인앱결제(IAP) 강제·심사 정책은 제약이 아니다(토스 외부결제 그대로 OK). 단 약관·개인정보·결제·구독·무료티어·알림 권한 등 **정책/기능 흐름은 완성도 있게 구현**한다 — "출시만 안 함"이지 "정책을 대충"이 아니다. 웹 랜딩 하단 홍보 섹션은 앱에선 버리고 위 4단계 + 검색창 메인으로 압축한다.
