...
텍스트 면접관은 **Qwen2.5-3B-Instruct를 LoRA 파인튜닝**하여 운용한다(2026-06-19 학습 완료, train_loss 0.71). 합성 데이터는 Claude 선생 증류로 생성했다. 서빙은 팀 공통 **Ollama**(merge → GGUF) 흐름을 따르며, 원격 RTX 4090에 상시 기동한다. (구안의 Qwen2.5-7B + vLLM 실험 스크립트는 폐기.) 상세 학습 절차는 `ml/interview-finetune/TRAINING.md`, 배포 토폴로지는 `docs/planning/면접 자율 에이전트 로드맵.md` 12장.
