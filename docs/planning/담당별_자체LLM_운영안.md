...
{
  "domain": "E",
  "model": "careertuner-e-correction-3b:latest",
...
ai:
  domains:
    ...
    e:
      ...
      model: ${E_AI_MODEL:careertuner-e-correction-3b:latest}
...
```json
{
  ...
  "E": { "model": "careertuner-e-correction-3b:latest", "available": true, "fallback": "anthropic-openai", "lastCheckedAt": "2026-06-16T09:00:00" },
...
| E | 첨삭/결제/크레딧 | 자기소개서·면접답변·이력서·포트폴리오 원문 보존형 첨삭 | Qwen2.5-3B LoRA 단일 모델, strict schema·repair·fallback | 개선안, 변경 이유, 위험 표현, 첨삭 이력 |
| F | 커뮤니티/고객센터/공지/알림 | 검열, 태깅, 신고분류, 면접질문 추출, 안내 챗봇 | Gemma 4, BGE-M3 | 검열 결과, 자동 태그, 신고 유형, 면접 질문, RAG 답변 |

공통 베이스 모델은 Qwen2.5-3B로 통일하며 E 첨삭도 같은 베이스를 사용한다. E는 별도 대형 모델 예외를 두지 않고 데이터 품질, 출력 계약, repair와 외부 API fallback으로 생성 품질을 보완한다.
...
E 파트의 베이스 모델은 팀 공통 `Qwen/Qwen2.5-3B-Instruct`로 확정한다. 자기소개서와 면접 답변을 실제 문장으로 고치는 생성형 task지만, Qwen3-8B·Qwen3-4B 예외 모델은 사용하지 않는다.

E는 Qwen2.5-3B 통합 모델 하나에 네 첨삭 task를 학습한다. 장문 보존, 허위 사실 차단, JSON 계약 문제는 데이터 커리큘럼, strict JSON Schema, 동일 모델 repair 1회와 서버 검증으로 다루고, 최종 안정성은 Anthropic과 OpenAI fallback으로 확보한다.
...
→ careertuner-e-correction-3b adapter 생성
→ 필요 시 merge GGUF로 변환
→ Ollama에서 careertuner-e-correction-3b:latest 태그로 서빙
```
...
| 우선순위 | 모델/방식 | 용도 | 판단 기준 |
| --- | --- | --- | --- |
| 1 | Qwen2.5-3B E LoRA 통합 모델 | E 파트 단일 자체 첨삭 모델 | 네 첨삭 task의 기본 실행 |
| 2 | strict JSON Schema·동일 3B repair | 구조·길이·문단·허위 사실 검증 | 첫 출력이 계약을 위반한 경우 1회 보정 |
| 3 | Anthropic 기반 첨삭 | 1차 외부 fallback | 자체 3B와 repair가 실패한 경우 |
| 4 | OpenAI 기반 첨삭 | 최종 외부 fallback | Anthropic이 실패하거나 미설정인 경우 |
...
```text
1. E 자체 모델 재시도 1회
2. 동일 Qwen2.5-3B E adapter에 repair 요청 1회
3. Anthropic 기반 첨삭
4. OpenAI 기반 첨삭
5. 최종 실패 상태 저장 + 사용자 재시도 안내
```
...
- [ ] 원문 보존 검증 케이스 작성
- [x] Qwen2.5-3B 단일 E 모델과 운영 태그 확정
...
특히 E 파트는 사용자가 실제 지원 문서에 반영할 문장을 다루므로, 모델 품질·원문 보존·허위 사실 방지·첨삭 이력 저장·크레딧 차감이 모두 중요하다. E는 팀 공통 Qwen2.5-3B를 단일 자체 모델로 사용하고, 데이터 보강과 서버 검증, Anthropic·OpenAI fallback으로 품질과 안정성을 확보한다.
