...
텍스트 면접관은 Qwen 계열 모델을 LoRA 파인튜닝하여 운용한다. 현재 `ml/interview-finetune`에는 Qwen2.5-7B + vLLM 기반 실험 README와 스크립트가 있으므로, D는 팀 공통 Ollama 흐름과 별개로 GPU 서버 실험을 유지할 수 있다.
...
```text
브라우저 MediaPipe
→ 표정 blendshape, 자세 landmark 추출
→ Web Audio·Whisper 기반 음성 특징 추출
→ LightGBM 평가 헤드
→ 종합 면접 인상 점수 예측
```
...
비언어 모델은 ONNX로 변환하여 브라우저 온디바이스 추론을 목표로 한다. 영상 원본은 서버로 전송하지 않고, 필요한 경우 특징벡터만 서버로 전송한다.
