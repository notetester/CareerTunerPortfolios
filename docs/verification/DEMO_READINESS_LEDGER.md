...
## COMPANY-ADS 신규 기능 (검증 예정)

이 절은 COMPANY-ADS 브랜치에서 배선한 채용공고 게시판·기업 서비스 허브·광고 슬롯의 검증 경계를 남긴다.
세 기능은 프런트 라우트·컴포넌트와 백엔드 도메인이 배선되어 있으나, 위 PR #448 전 영역 스윕에는 포함되지 않았다.
`dev` 병합 시 strict selector와 backend/frontend CI로 재검증하고, 라이브 배포·자동 증거가 확보되면 아래 표를
증거 ID와 함께 `PASS`/`PASS_TARGETED`로 승격한다. 현재는 확인된 CI·배포 증거가 없으므로 `PENDING_LIVE`로 둔다.

| 체크 | 상태 | 확인할 경계 | 증거 |
| --- | --- | --- | --- |
| JOBBOARD | PENDING_LIVE | 채용공고 게시판 목록·상세(`/api/job-board`), 검수 통과 공고만 노출, 지원 건 가져오기, 목록 내 광고 슬롯 렌더, 폭·반응형 | dev 병합·CI 후 갱신 |
| COMPANY-HUB | PENDING_LIVE | 기업 계정 공고 등록·수정·마감(`/api/company/job-postings`), 검수 상태머신, `company/manage` 인증 게이트, 공개 회사/브랜드 소개와 분리, 운영측 `admin/company` 공고 검수 | dev 병합·CI 후 갱신 |
| ADS | PENDING_LIVE | 광고 슬롯 노출·클릭 집계(`/api/ads`), 이미지 서빙, 관리자 광고 등록·집행(`admin/ads`) | dev 병합·CI 후 갱신 |

이 절은 배선 사실만 기록하며 검증 완료를 뜻하지 않는다. 세 기능이 전 영역 스윕에 편입되면 위 A~F 및 플랫폼
체크리스트와 실행 증거 표에 정식 행으로 옮기고 이 임시 절은 정리한다.
