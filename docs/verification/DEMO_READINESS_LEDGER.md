...
| 현재 시연·배포 기준 | PR #392, merge `6e7ed22b3a000dafb0249d241994d7aaf4681ebf` |
| 검증일 | 2026-07-12 |
| 변경 기준 | PR #390 merge `7364c4e51e137453e89044e19d0a684c66021174` 이후 PR #391 전체 보강 + PR #392 배포 경계 표적 재검증 |
...
| `DEPLOY-LIVE-01` | `PASS` | backend/DB run `29189589610`: 관리형 DB 패치 준비·전송·적용, backend/worker image 배포, EC2 readiness까지 성공 |
| `WEB-LIVE-01` | `PASS` | PR #392, run `29190198851`: TLS server root `/var/www/careertuner`, AASA origin 사전검증, 웹 교체, 공개 Asset Links/AASA 본문·JSON MIME 검증. 외부 재확인에서 `/`, health, readiness 모두 200·DB `UP` |
...
| `SITES-BACKUP-01` | `PASS` | Sites source `2002c6d62a394ed1bf64b284d094d0b2906b4275`, version 4를 owner-only 백업 `https://careertuner.career-tuner-4654.chatgpt.site`에 게시하고 deployment 성공 확인 |
...
| PLATFORM-WEB | PASS | mock/AWS 빌드, 영향 경로 실브라우저, 반응형·다크·권한 경계, AWS live·Sites 백업 배포 | FE-ALL-01, WEB-UI-01, WEB-LIVE-01, SITES-BACKUP-01 |
...
| RELEASE-GATE | PASS | PR #391·#392 merge, backend/DB·web·Android live·Sites owner-only 백업 배포와 공개 health/App Links 확인 | CI-PR391-01, DEPLOY-LIVE-01, WEB-LIVE-01, ANDROID-LIVE-01, SITES-BACKUP-01 |
