...
| 역할 | 접속 방법 | 볼 수 있는 것 |
| --- | --- | --- |
| **일반 사용자** (김데모) | 로그인 화면 → "일반 사용자로 체험", 또는 `demo@careertuner.dev` / `demo1234` | 대시보드·지원 건·공고 분석·적합도·AI 가상 면접·첨삭·커뮤니티·결제 등 사용자 전 기능 |
| **관리자** (한관리) | 로그인 화면 → "관리자로 체험", 또는 `admin@careertuner.dev` / `demo1234` | 위 + **관리자 콘솔**: 부여된 READ/CREATE/UPDATE/DELETE 권한에 맞춘 메뉴·버튼·API |
