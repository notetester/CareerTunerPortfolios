...
데모에는 **일반 사용자**와 **관리자** 두 역할이 준비되어 있습니다. 로그인 화면 상단의 **"데모 빠른 체험"** 패널에서 원클릭으로 전환하거나, 위 딥링크(`?demo=user` / `?demo=admin`)로 바로 들어갈 수 있습니다.

| 역할 | 접속 방법 | 볼 수 있는 것 |
| --- | --- | --- |
| **일반 사용자** (김데모) | 로그인 화면 → "일반 사용자로 체험", 또는 `demo@careertuner.dev`와 임의 비밀번호 | 대시보드·지원 건·공고 분석·적합도·AI 가상 면접·첨삭·커뮤니티·결제 등 사용자 전 기능 |
| **관리자** (한관리) | 로그인 화면 → "관리자로 체험", 또는 `admin@careertuner.dev`와 임의 비밀번호 | 위 + **관리자 콘솔**: 부여된 READ/CREATE/UPDATE/DELETE 권한에 맞춘 메뉴·버튼·API |
...
└─ .github/workflows/pages.yml  데모 + 설명서 조립·배포(시크릿 스캔 게이트)
```
